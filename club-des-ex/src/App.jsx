...
const emptyDepot = (date) => ({
  date, lotoQuebec:"", coffre:"", guichet:"", caisse:"", mathieu:"", soumisTs:"", soumisePar:"",
});
...
function DepotPage({ onBack, sheet, setSheet, showToast }) {
  ...
  const mathieu   = parseFloat(depot.mathieu)    || 0;
  const sumParts = Math.round((coffre + guichet + caisse) * 100) / 100;
  const totalNet  = Math.round((loto - mathieu) * 100) / 100;
  const balanced = loto > 0 && Math.abs(sumParts - totalNet) < 0.01;
  const hasError = loto > 0 && sumParts > 0 && !balanced;

  const handleSave = () => {
    if (hasError) { showToast("⛔ Le total Coffre + Guichet + Caisse doit égaler Loto-Québec − Dépôt Mathieu", "err"); return; }
    const updated = {
      ...depot,
      ...
      <div className="main">
        ...
        <div className="card">
          ...
          <div className="card-body" style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            {/* Dépôt Mathieu — soustrait du total Loto-Québec */}
            <div style={{borderTop:"1px dashed var(--border)",paddingTop:12}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                <div>
                  <div style={{fontWeight:600,fontSize:".9rem",color:"var(--accent)"}}>👤 Dépôt Mathieu</div>
                  <div style={{fontSize:".72rem",color:"var(--muted)"}}>Montant retiré par Mathieu — soustrait du total Loto-Québec</div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{color:"var(--muted)"}}>$</span>
                  <input type="number" min="0" step="0.01"
                    ...
                    disabled={isLocked || (!canEdit)}
                    style={{width:130,textAlign:"right"}}
                    onChange={e=>setF("mathieu",e.target.value)}
                    placeholder="0.00" />
                </div>
              </div>
            </div>
            ...
            <div style={{borderTop:"2px solid var(--border)",paddingTop:12,
              ...
              <div>
                ...
                <div style={{fontSize:".75rem",color:"var(--muted)"}}>Doit égaler Loto-Québec ({loto.toFixed(2)} $) − Mathieu ({mathieu.toFixed(2)} $) = {totalNet.toFixed(2)} $</div>
                {hasError && <div style={{color:"var(--red)",fontSize:".78rem",marginTop:2}}>
                  ⛔ Doit égaler {totalNet.toFixed(2)} $ (Loto {loto.toFixed(2)} $ − Mathieu {mathieu.toFixed(2)} $) — écart : {(sumParts-totalNet).toFixed(2)} $
                </div>}
  ...
  const setVin = (id, field, rawVal) => {
    setSheet(s => {
      ...
      if (info.type === "vinier") {
        ...
        item["vente"+shift] = diff > 0 ? Math.round(diff / (info.grPerVerre||250) * 2) / 2 : 0;
      } else {
      ...
      if (isManagerPin || authorizedEmp) {
        ...
        setSheet(s => {
          ...
          if (info?.type === "vinier") {
            ...
            item["vente"+shift] = diffAvant > 0 ? Math.round(diffAvant / (info.grPerVerre||250) * 2) / 2 : 0;
          }
...
function EmployeeCaisse({ sheet, setSheet, shift, employee, showToast, prevCaisseTotal }) {
  ...
  return (
    <>
      {!isSubmitted && (
        <div style={{background:"rgba(239,68,68,.15)",border:"1px solid var(--red)",
          borderRadius:8,padding:"12px 16px",marginBottom:12,
          display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:"1.3rem"}}>⚠️</span>
          <div>
            <div style={{fontWeight:700,color:"var(--red)"}}>Caisse non soumise</div>
            <div style={{fontSize:".78rem",color:"var(--muted)"}}>
              N'oubliez pas de soumettre votre caisse avant de partir !
            </div>
          </div>
        </div>
      )}
      ...
      <div className="card">
        ...
        <div className="card-body">
          {row("💵 Argent reçu", c.argentRecu, "argentRecu",
            shift === "PM"
              ? `Report automatique — Total caisse shift AM${sheet.caisse?.AM?.totalCaisse ? ` (${parseFloat(sheet.caisse.AM.totalCaisse).toFixed(2)} $)` : ""}${isManagerUnlocked() ? " — ✏️ Modifiable (gérant)" : ""}`
              : `Report automatique — Total caisse shift PM de la veille${prevCaisseTotal ? ` (${parseFloat(prevCaisseTotal).toFixed(2)} $)` : ""}${isManagerUnlocked() ? " — ✏️ Modifiable (gérant)" : ""}`,
            !isManagerUnlocked() // lecture seule pour barmaid, modifiable pour gérant
          )}
      ...
      return (
        <div style={{padding:30,background:"#1a1a1a",minHeight:"100vh",color:"#fff"}}>
          ...
          <button onClick={()=>window.location.reload()}
            style={{marginTop:20,padding:"10px 20px",background:"#e8a020",
            border:"none",borderRadius:8,cursor:"pointer",fontWeight:700,color:"#000"}}>
            🔄 Recharger
          </button>
        </div>
      );
