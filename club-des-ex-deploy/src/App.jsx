...
function EmployeeCaisse({ sheet, setSheet, shift, employee, showToast, prevCaisseTotal }) {
  ...
  // Auto-fill argentRecu from previous shift total — always sync when prevCaisseTotal changes
  useEffect(() => {
    if (prevCaisseTotal === "" || prevCaisseTotal === undefined) return;
    // Only auto-fill if not yet submitted (soumisePar empty) or if value is empty
    if (c.argentRecu === "" || c.argentRecu === undefined) {
      setSheet(s => {
...
function AppPasswordGate({ onUnlock }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",
      ...
      <div style={{fontFamily:"Georgia,serif",fontSize:"2rem",color:"#e8a020",letterSpacing:".1em"}}>
        LE CLUB DES EX
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12,width:"100%",maxWidth:300}}>
        <input type="password" value={input} placeholder="Mot de passe..." autoFocus
          style={{fontSize:"1.1rem",padding:"12px 16px",textAlign:"center",
            background:"#1a1a1a",border:"1px solid "+(error?"#ef4444":"#333"),
            borderRadius:8,color:"#fff",outline:"none"}}
          onChange={e=>setInput(e.target.value)}
          onKeyDown={e=>e.key==="Enter"&&tryUnlock()} />
        {error && (
          <div style={{color:"#ef4444",textAlign:"center",fontSize:".82rem"}}>
            ⛔ Mot de passe incorrect
          </div>
        )}
        <button style={{padding:"12px",fontSize:"1rem",background:"#e8a020",border:"none",
...
export default function App() {
  ...
  // ── Firebase init + sync automatique ──────────────────────────────────────
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Toutes les clés à synchroniser depuis Firebase
        const localKeys = Object.keys(localStorage).filter(k =>
          k.startsWith("sheet_") || k.startsWith("depot_")
        );
        ...
        // Écoute temps réel pour les clés critiques
        const listenKeys = ["sheet_"+today, "employees", "inventory", "prices", "manager_pin"];
        listenKeys.forEach(key => {
          const unsub = fbListen(key, (val) => {
            const cur = localStorage.getItem(key);
            if (JSON.stringify(val) !== cur) {
              localStorage.setItem(key, JSON.stringify(val));
              // Recharge uniquement si la feuille du jour ou employés changent
        ...
        console.warn("[Firebase] Mode hors-ligne:", e.message);
      }
    };
    ...
    <>
      ...
      {!online && (
        <div style={{position:"fixed",top:0,left:0,right:0,zIndex:9999,
          ...
          📵 Hors-ligne — les données seront synchronisées automatiquement à la reconnexion
        </div>
      )}
