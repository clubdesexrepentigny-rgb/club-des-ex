...
const emptyDaySheet = (date) => ({
  ...
  caisse: {
    AM: {
      argentRecu:     "",   // report de la caisse précédente (auto-rempli)
      machinesSous:   "",   // gérant seulement
      argentCoffre:   "",   // barmaid entre
      coupons:        "",   // barmaid entre (montant à déduire)
      venteCalculee:  0,    // calculé auto
      totalCaisse:    0,    // calculé auto = argentRecu + machinesSous + argentCoffre - coupons + venteCalculee
      soumisePar:     "",
      soumiseTs:      "",
      note:           "",
    },
...
const migrateSheet = (sheet) => {
  ...
  const emptyCaisseDay = {
    argentRecu:"", machinesSous:"", argentCoffre:"", coupons:"",
    ...
    if (isToday && depot.caisse !== "") {
      setSheet(s => {
        ...
        newCaisse.AM.totalCaisse = Math.round((
          ...
          (parseFloat(c.argentCoffre)||0) - (parseFloat(c.coupons)||0) + vente
        ) * 100) / 100;
...
function EmployeeCaisse({ sheet, setSheet, shift, employee, showToast, prevCaisseTotal }) {
  ...
  const setC = (field, val) => {
    setSheet(s => {
      ...
      caisse[shift].totalCaisse   = Math.round((recu + machines + coffre - coupons + venteCalculee)*100)/100;
      return { ...s, caisse };
    });
  };
  ...
  const handleSubmit = () => {
    setSheet(s => {
      ...
      caisse[shift].totalCaisse = Math.round((
        ...
        (parseFloat(cur.argentCoffre)||0) - (parseFloat(cur.coupons)||0) + venteCalculee
      )*100)/100;
      return { ...s, caisse };
    });
    setEditMode(false);
  ...
  const total = Math.round((
    ...
    (parseFloat(c.argentCoffre)||0) - (parseFloat(c.coupons)||0) + venteCalculee
  )*100)/100;
...
function ManagerVerifCaisse({ sheet, setSheet, history, setHistory, showToast }) {
  ...
  const shiftCard = (shift) => {
    ...
    const total   = Math.round((recu + machines + coffre - coupons + vente)*100)/100;
...
// ─── MOT DE PASSE APP ─────────────────────────────────────────────────────────
// Change cette valeur pour ton mot de passe secret
...
function AppPasswordGate({ onUnlock }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",
      justifyContent:"center",minHeight:"100vh",background:"var(--bg)",gap:24,padding:20}}>
      <div style={{fontFamily:"var(--font-head)",fontSize:"2.2rem",
        color:"var(--accent)",letterSpacing:".1em",textAlign:"center"}}>
        LE CLUB DES EX
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12,width:"100%",maxWidth:300}}>
        <input
          ...
          autoFocus
          style={{
            fontSize:"1.1rem",padding:"12px 16px",
        ...
        />
        {error && (
          <div style={{color:"var(--red)",textAlign:"center",fontSize:".82rem"}}>
            ⛔ Mot de passe incorrect
          </div>
        )}
        <button className="btn btn-accent" style={{padding:"12px",fontSize:"1rem"}}
          onClick={tryUnlock}>
          Entrer →
        </button>
      </div>
...
export default function App() {
  ...
  // Firebase init + sync en arrière-plan (n'attend pas avant d'afficher l'app)
  useEffect(() => {
    ...
    const init = async () => {
      try {
        await initAuth();
        // Sync Firebase → localStorage en arrière-plan
        ...
        console.warn("Firebase offline, using local data", e);
      }
    };
    // Lance Firebase en arrière-plan — l'app s'affiche immédiatement
  ...
  return (
    <>
      ...
      {screen==="loading" && (
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",
          justifyContent:"center",minHeight:"100vh",background:"var(--bg)",gap:20}}>
          <div style={{fontFamily:"var(--font-head)",fontSize:"2rem",
            color:"var(--accent)",letterSpacing:".1em"}}>LE CLUB DES EX</div>
          <div style={{color:"var(--muted)"}}>Connexion en cours...</div>
