    ...
    <div className="app">
      ...
      <div className="main">
        ...
        {(() => {
          ...
          const [newItem, setNewItem] = React.useState({item:"",qty:1,note:""});

          const addOfferte = () => {
            if (!newItem.item) return;
            const found = ALL_ITEMS.find(i=>i.id===newItem.item);
            if (!found) return;
            const prix = (parseFloat(prices[newItem.item])||0) * (parseFloat(newItem.qty)||1);
            const entry = {
              id: Date.now().toString(),
              shift, item: newItem.item,
              label: found.label, cat: found.cat,
              qty: parseFloat(newItem.qty)||1,
              prix, note: newItem.note,
              approuve: null, // en attente
              ts: new Date().toLocaleString("fr-CA"),
            };
            setSheet(s=>({...s, offertes:[...(s.offertes||[]), entry]}));
            setNewItem({item:"",qty:1,note:""});
            setShowForm(false);
          };
                ...
                {!shiftReadOnly && (
                  showForm ? (
                    <div style={{display:"flex",flexDirection:"column",gap:8,
                      ...
                      <select value={newItem.item} style={{width:"100%"}}
                        onChange={e=>setNewItem(f=>({...f,item:e.target.value}))}>
                      ...
                      <div style={{display:"flex",gap:8}}>
                        <div style={{flex:1}}>
                          ...
                          <input type="number" min="1" step="1" value={newItem.qty} style={{width:"100%"}}
                            onChange={e=>setNewItem(f=>({...f,qty:e.target.value}))} />
                        </div>
                        <div style={{flex:2}}>
                          <div style={{fontSize:".72rem",color:"var(--muted)",marginBottom:2}}>Note (optionnel)</div>
                          <input type="text" value={newItem.note} placeholder="Ex: table 5, ami de..."
                            style={{width:"100%"}}
                            onChange={e=>setNewItem(f=>({...f,note:e.target.value}))} />
                        </div>
                      </div>
                      <div style={{display:"flex",gap:8}}>
        ...
        {/* ── ALERTE ERREUR ── */}
        {(() => {
          const [showErrForm, setShowErrForm] = React.useState(false);
