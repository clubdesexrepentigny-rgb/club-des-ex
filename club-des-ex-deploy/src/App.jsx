    ...
    if (this.state.error) {
      return (
        <div style={{padding:30,background:"#1a1a1a",minHeight:"100vh",color:"#fff"}}>
          <h2 style={{color:"#ef4444"}}>⛔ Erreur dans l'application</h2>
          <p style={{color:"#fbbf24",fontFamily:"monospace",fontSize:".85rem",whiteSpace:"pre-wrap"}}>
            {this.state.error?.message}
          </p>
          <p style={{color:"#999",fontFamily:"monospace",fontSize:".72rem",whiteSpace:"pre-wrap"}}>
            {this.state.error?.stack?.slice(0,600)}
          </p>
          <button onClick={()=>window.location.reload()}
            style={{marginTop:20,padding:"10px 20px",background:"#e8a020",border:"none",
            borderRadius:8,cursor:"pointer",fontWeight:700,color:"#000"}}>
            🔄 Recharger
          </button>
        </div>
      );
...
function AppPasswordGate({ onUnlock }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",
      justifyContent:"center",minHeight:"100vh",background:"#0f0f0f",gap:24,padding:20}}>
      <div style={{fontFamily:"Georgia,serif",fontSize:"2rem",color:"#e8a020",
        letterSpacing:".1em",textAlign:"center"}}>LE CLUB DES EX</div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12,width:"100%",maxWidth:300}}>
        <input type="password" value={input} placeholder="Mot de passe..." autoFocus
          style={{fontSize:"1.1rem",padding:"12px 16px",textAlign:"center",
            background:"#1a1a1a",border:"1px solid "+(err?"#ef4444":"#333"),
            borderRadius:8,color:"#fff",outline:"none"}}
          onChange={e=>setInput(e.target.value)}
          onKeyDown={e=>e.key==="Enter"&&tryUnlock()} />
        {err && <div style={{color:"#ef4444",textAlign:"center",fontSize:".82rem"}}>
          ⛔ Mot de passe incorrect
        </div>}
        <button style={{padding:"12px",fontSize:"1rem",background:"#e8a020",
          border:"none",borderRadius:8,color:"#000",fontWeight:700,cursor:"pointer"}}
          onClick={tryUnlock}>Entrer →</button>
      </div>
    </div>
  );
}
    ...
    const init = async () => {
      try {
        ...
        const localKeys = Object.keys(localStorage)
          .filter(k => k.startsWith("sheet_") || k.startsWith("depot_"));
        const syncKeys = [...new Set([
          "manager_pin","employees","inventory","history",
          "prices","custom_products","cocktails_v2",
          "sheet_"+today, ...localKeys
        ])];
        ...
        ["sheet_"+today,"employees","inventory","prices","manager_pin"].forEach(key => {
          const unsub = fbListen(key, (val) => {
            ...
            if (JSON.stringify(val) !== cur) {
              localStorage.setItem(key, JSON.stringify(val));
              if (key === "sheet_"+today || key === "employees")
    ...
    <>
      ...
      {!online && (
        <div style={{position:"fixed",top:0,left:0,right:0,zIndex:9999,
          ...
          fontSize:".75rem",padding:"5px",fontWeight:600}}>
          📵 Hors-ligne — synchronisation automatique à la reconnexion
        </div>
      )}
      {!appUnlocked ? (
        <AppPasswordGate onUnlock={() => {
