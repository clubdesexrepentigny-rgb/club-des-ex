...
import { initAuth, fbSet, fbGet, fbListen } from "./firebase.js";
...
const VIN_ITEMS = [
  ...
  { id:"vinier_blanc1", label:"Vinier Blanc Cliff 79 #1", type:"vinier", grPerVerre:250, poidsFull:4350 },
  { id:"vinier_blanc2", label:"Vinier Blanc Cliff 79 #2", type:"vinier", grPerVerre:250, poidsFull:4350 },
  { id:"vinier_bistro1",label:"Vinier Blanc Bistro #1",   type:"vinier", grPerVerre:250, poidsFull:4350 },
  { id:"vinier_bistro2",label:"Vinier Blanc Bistro #2",   type:"vinier", grPerVerre:250, poidsFull:4350 },
...
const emptyDaySheet = (date) => ({
  ...
  // Promos — seulement pour les barmaids autorisées
  promoShooter:   { AM:"", PM:"" }, // shooters vendus à 2$ (bug7/8/9)
  promoBiere:     { AM:"", PM:"" }, // petites bières vendues à 4$
  promoVin:       { AM:"", PM:"" }, // verres de vinier vendus à 5$
  // Cocktails — dynamic, keyed by cocktail id
  cocktails: {},
  // Consommations offertes par le gérant
  offertes: [], // [{id, shift, item, qty, prix, note, approuve: null}]
  alertesErreur: [], // [{id, shift, texte, soumisePar, ts, statut: null/true/false}]
...
const migrateSheet = (sheet) => {
  ...
  if (!s.promoShooter) s.promoShooter = { AM:"", PM:"" };
  if (!s.promoBiere)   s.promoBiere   = { AM:"", PM:"" };
  if (!s.promoVin)     s.promoVin     = { AM:"", PM:"" };
  if (!s.cocktails) s.cocktails = {};
  if (!s.offertes) s.offertes = [];
  if (!s.alertesErreur) s.alertesErreur = [];
...
// ── Session gérant — une fois déverrouillé, reste valide toute la session ────
const MANAGER_SESSION_KEY = "mgr_unlocked";
const isManagerUnlocked = () => sessionStorage.getItem(MANAGER_SESSION_KEY) === "1";
const unlockManager     = () => sessionStorage.setItem(MANAGER_SESSION_KEY, "1");
const lockManager       = () => sessionStorage.removeItem(MANAGER_SESSION_KEY);

// Vérifie un PIN gérant et déverrouille la session si correct
const checkManagerPin = (np) => {
...
const canUsePromos       = (emp) => emp?.role === "gerant" || emp?.role === "manageuse" || emp?.permissions?.promos === true;

const emptyDepot = (date) => ({
  date, lotoQuebec:"", coffre:"", guichet:"", caisse:"", depotMathieu:"", soumisTs:"", soumisePar:"",
});
...
function DepotPage({ onBack, sheet, setSheet, showToast }) {
  ...
  const [pinUnlocked, setPinUnlocked] = useState(() => isManagerUnlocked());
  ...
  const loadDate = (date) => {
    ...
    setEditMode(false);
    setPinUnlocked(date === today || isManagerUnlocked()); // today or already auth
    setPinInput("");
    setPinErr(false);
  };
  ...
  const depotMathieu = parseFloat(depot.depotMathieu)|| 0;
  const sumParts = Math.round((coffre + guichet + caisse - depotMathieu) * 100) / 100;
  const balanced = loto > 0 && Math.abs(sumParts - loto) < 0.01;
  const hasError = loto > 0 && sumParts > 0 && !balanced;

  const handleSave = () => {
    if (hasError) { showToast("⛔ Le total Coffre + Guichet + Caisse − Dépôt Mathieu doit égaler Loto-Québec", "err"); return; }
    const updated = {
      ...depot,
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div style={{fontSize:".8rem",color:"var(--muted)",marginBottom:10}}>
              Total retiré des machines. Le total Coffre + Guichet + Caisse − Dépôt Mathieu doit être égal.
            </div>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{display:"flex",flexDirection:"column",gap:16}}>
            {[
              ...
              { field:"depotMathieu",  label:"💼 Dépôt Mathieu",   hint:"Montant déduit du total (dépôt personnel)" },
            ].map(item => (
              <div key={item.field}>
            ...
            <div style={{borderTop:"2px solid var(--border)",paddingTop:12,
              ...
              <div>
                <div style={{fontWeight:700}}>Total Coffre + Guichet + Caisse − Dépôt Mathieu</div>
...
function EmployeeView({ sheet, setSheet, prevFinsPM, employee, onLogout, showToast,
  ...
  // Date picker — gérant only, barmaids cannot access past dates
  const [datePinUnlocked, setDatePinUnlocked] = useState(
    () => isManagerUnlocked() && isGerantUser
  );
  ...
  // Offertes section state
  const [showOfferteForm, setShowOfferteForm] = useState(false);
  const [newOfferte, setNewOfferte] = useState({item:"",qty:1,note:""});
  // Alerte erreur state
  ...
  const openDatePicker = () => {
    if (!isGerantUser) {
      showToast("⛔ Seul le gérant peut consulter les dates passées", "err");
      return;
    }
  ...
  const setBug = (id, field, rawVal) => {
    setSheet(s => {
      ...
      item[field] = rawVal;
      // Recalc vente — utilise prevFinsPM si debutAM est verrouillé/vide
      const debutRaw = item["debut"+shift] !== "" && item["debut"+shift] !== undefined
        ? item["debut"+shift]
        : (shift === "AM" ? prevFinsPM?.bugs?.[id] : item.finAM);
      const debut = parseFloat(debutRaw);
  ...
  const onFinAMBlurBug = (id) => {
    setSheet(s => {
      const bugs = { ...s.bugs };
      const item = { ...bugs[id] };
      // Auto-fill finAM if empty — use debutAM (nothing sold/moved)
      if ((item.finAM === "" || item.finAM === undefined) &&
          (item.debutAM !== "" && item.debutAM !== undefined)) {
        item.finAM  = item.debutAM;
        item.venteAM = 0;
      }
      // Lock debutPM = finAM
      if (item.finAM !== "" && item.finAM !== undefined) {
        item.debutPM = item.finAM;
        const debut = parseFloat(item.debutPM);
        const fin   = parseFloat(item.finPM);
        if (!isNaN(debut) && !isNaN(fin)) {
          item.ventePM = fin >= debut ? fin - debut : (999 - debut) + fin + 1;
        }
      }
      bugs[id] = item;
      return { ...s, bugs };
  ...
  const setVin = (id, field, rawVal) => {
    setSheet(s => {
      ...
      item[field] = rawVal;
      // Utilise prevFinsPM si debutAM est verrouillé/vide
      const debutVinRaw = item["debut"+shift] !== "" && item["debut"+shift] !== undefined
        ? item["debut"+shift]
        : (shift === "AM" ? prevFinsPM?.vins?.[id] : item.finAM);
      const debut = parseFloat(debutVinRaw) || 0;
  ...
  const onFinAMBlurVin = (id) => {
    setSheet(s => {
      ...
      const item = { ...vins[id] };
      const info = VIN_ITEMS.find(v=>v.id===id);
      // Auto-fill finAM if empty — use debutAM (nothing sold)
      if ((item.finAM === "" || item.finAM === undefined) &&
          (item.debutAM !== "" && item.debutAM !== undefined)) {
        item.finAM  = item.debutAM;
        item.venteAM = 0;
      ...
      if (item.finAM !== "" && item.finAM !== undefined) {
        item.debutPM = item.finAM;
        const debut = parseFloat(item.debutPM) || 0;
        const fin   = parseFloat(item.finPM)   || 0;
        if (info?.type === "vinier") {
          const diff = debut - fin;
          item.ventePM = diff > 0 ? Math.round(diff / (info.grPerVerre||250) * 2) / 2 : 0;
        } else {
          item.ventePM = debut > fin ? debut - fin : 0;
        }
      }
      vins[id] = item;
      return { ...s, vins };
    ...
    <div className="app">
      <div className="topbar">
        ...
        <button className="btn btn-ghost" onClick={() => {
          // Check if caisse is submitted before logging out
          const caisseAM = sheet.caisse?.AM;
          const caissePM = sheet.caisse?.PM;
          const amOk = !caisseAM || !!caisseAM.soumiseTs;
          const pmOk = !caissePM || !!caissePM.soumiseTs;
          const myShiftSubmitted = shift === "AM" ? amOk : pmOk;
          if (!myShiftSubmitted && isViewingToday) {
            if (!window.confirm("⚠️ Ta caisse n'a pas été soumise!\n\nEs-tu sûr(e) de vouloir quitter sans soumettre ta caisse ?")) return;
          }
          onLogout();
        }}>Quitter</button>
      </div>
      ...
      <div className="main">
          ...
          <div className="card-body" style={{padding:0}}>
            <table>
              <thead>
                <tr>
                  <th>Article</th>
                  <th style={{textAlign:"center",width:75}}>Déb.AM</th>
                  <th style={{textAlign:"center",width:75}}>Fin AM</th>
                  <th style={{textAlign:"center",width:65,color:"var(--accent)"}}>Vte AM</th>
                  <th style={{textAlign:"center",width:75}}>Déb.PM</th>
                  <th style={{textAlign:"center",width:75}}>Fin PM</th>
                  <th style={{textAlign:"center",width:65,color:"var(--accent)"}}>Vte PM</th>
                </tr>
              </thead>
              <tbody>
                {BUG_ITEMS.map(bug => {
                  const d = sheet.bugs[bug.id] || {};
                  const prevFin = prevFinsPM?.bugs?.[bug.id];
                  const debutAMLocked = prevFin !== undefined && prevFin !== "";
                  const debutPMLocked = d.finAM !== "" && d.finAM !== undefined;
                  const venteAM = parseFloat(d.venteAM)||0;
                  const ventePM = parseFloat(d.ventePM)||0;
                  const inp = (val, field, locked) => (
                    <input type="number" min="0" value={val||""}
                      readOnly={locked} className={locked?"locked":""}
                      style={{width:65,textAlign:"center"}}
                      onChange={e=>setBug(bug.id,field,e.target.value)}
                      onBlur={field==="finAM"?()=>onFinAMBlurBug(bug.id):undefined} />
                  );
                  return (
                    <tr key={bug.id}>
                      <td className="item-name" style={{fontSize:".78rem"}}>
                        {bug.label}
                        <span style={{fontSize:".65rem",color:"var(--muted)",display:"block"}}>{bug.oz} oz</span>
                      </td>
                      <td style={{textAlign:"center"}}>
                        {inp(d.debutAM,"debutAM", debutAMLocked||shift==="PM"||(shift==="AM"&&shiftReadOnly))}
                        {debutAMLocked&&<div style={{fontSize:".6rem",color:"var(--orange)"}}>🔒{prevFin}</div>}
                      </td>
                      <td style={{textAlign:"center"}}>
                        {inp(d.finAM,"finAM", shift==="PM"||(shift==="AM"&&shiftReadOnly))}
                      </td>
                      <td style={{textAlign:"center",fontWeight:600,color:venteAM>0?"var(--accent)":"var(--muted)"}}>
                        {venteAM||"—"}
                      </td>
                      <td style={{textAlign:"center"}}>
                        {inp(d.debutPM,"debutPM", debutPMLocked||shift==="AM"||(shift==="PM"&&shiftReadOnly))}
                        {debutPMLocked&&<div style={{fontSize:".6rem",color:"var(--muted)"}}>🔒{d.finAM}</div>}
                      </td>
                      <td style={{textAlign:"center"}}>
                        {inp(d.finPM,"finPM", shift==="AM"||(shift==="PM"&&shiftReadOnly))}
                      </td>
                      <td style={{textAlign:"center",fontWeight:600,color:ventePM>0?"var(--accent)":"var(--muted)"}}>
                        {ventePM||"—"}
                      </td>
                    </tr>
                  );
        ...
        {/* ── PLATEAUX DE SHOOTERS — après Bugs ── */}
        {(() => {
          ...
          const setPlat = (val) => setSheet(s => ({ ...s, plateauxShooter: { ...s.plateauxShooter, [shift]: val } }));
          return (
            <div className="card">
              <div className="card-header">
                <h3>🥃 Plateaux de Shooters</h3>
                <span style={{marginLeft:"auto",fontSize:".72rem",color:"var(--muted)"}}>1 plateau = 10 shooters · -10$ par plateau</span>
              </div>
              <div className="card-body">
                <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                  <div>
                    <div style={{fontSize:".74rem",color:"var(--muted)",marginBottom:4}}>Plateaux vendus — shift {shift}</div>
                    <input type="number" min="0" max={maxPlateaux} value={platVal}
                      readOnly={shiftReadOnly} className={shiftReadOnly?"locked":""}
                      style={{width:120,borderColor:platError?"var(--red)":undefined}}
                      onChange={e=>setPlat(e.target.value)} placeholder="0" />
                  </div>
                  <div style={{fontSize:".78rem",color:"var(--muted)"}}>
                    Shooters ce shift : <strong>{totalShootersShift}</strong> → max <strong>{maxPlateaux}</strong>
                  </div>
                  {platNum>0&&!platError&&<div style={{fontSize:".78rem",color:"var(--orange)"}}>→ Déduction : <strong>-{(platNum*10).toFixed(2)} $</strong></div>}
                </div>
                {platError&&<div style={{color:"var(--red)",fontSize:".78rem",marginTop:8}}>⛔ Max {maxPlateaux} plateau{maxPlateaux>1?"x":""}</div>}
              </div>
            </div>
          );
        })()}
        ...
        <div className="card">
          <div className="card-header"><h3>🍷 Vins & Spiritueux</h3>
            <span style={{marginLeft:"auto",fontSize:".7rem",color:"var(--muted)"}}>🔒 Début AM = Fin PM veille · Début PM = Fin AM</span>
          </div>
          <div className="card-body" style={{padding:0}}>
            <table>
              <thead>
              ...
              <tbody>
                {VIN_ITEMS.map(vin => {
                  ...
                  const venteAM = parseFloat(d.venteAM)||0;
                  const ventePM = parseFloat(d.ventePM)||0;
                  const isVinier = vin.type === "vinier";
                  const inp = (val, field, locked) => (
                    <input type="number" min="0"
                      ...
                      readOnly={locked} className={locked?"locked":""}
                      style={{width:65,textAlign:"center"}}
                      onChange={e=>setVin(vin.id,field,e.target.value)}
                      onBlur={field==="finAM"?()=>onFinAMBlurVin(vin.id):undefined} />
                  );
                  return (
                    <tr key={vin.id}>
                      <td className="item-name" style={{fontSize:".78rem"}}>{vin.label}</td>
                      <td style={{textAlign:"center"}}>
                        {inp(d.debutAM,"debutAM", debutAMLocked||shift==="PM"||(shift==="AM"&&shiftReadOnly))}
                        {debutAMLocked&&<div style={{fontSize:".6rem",color:"var(--orange)"}}>🔒{prevFin}</div>}
                      </td>
                ...
                <table>
                  ...
                  <tbody>
                    {customProds.map(p => {
                      ...
                      const amVal = d.am || ""; const pmVal = d.pm || "";
                      ...
                      return (
                        <tr key={p.id}>
                          ...
                          <td><input type="number" min="0" value={amVal}
                            readOnly={shift==="PM"||(shift==="AM"&&shiftReadOnly)} className={(shift==="PM"||(shift==="AM"&&shiftReadOnly))?"locked":""}
                            onChange={e=>{const v=e.target.value;setSheet(s=>({...s,customProducts:{...s.customProducts,[p.id]:{...d,am:v,total:(parseFloat(v)||0)+(parseFloat(pmVal)||0)}}}));}}/></td>
                          <td><input type="number" min="0" value={pmVal}
                            readOnly={shift==="AM"||(shift==="PM"&&shiftReadOnly)} className={(shift==="AM"||(shift==="PM"&&shiftReadOnly))?"locked":""}
                            onChange={e=>{const v=e.target.value;setSheet(s=>({...s,customProducts:{...s.customProducts,[p.id]:{...d,pm:v,total:(parseFloat(amVal)||0)+(parseFloat(v)||0)}}}));}}/></td>
        ...
        {(() => {
          ...
          return (
            <div className="card">
              <div className="card-header">
                <h3>🍹 Cocktails</h3>
                <span style={{marginLeft:"auto",fontSize:".72rem",color:"var(--muted)"}}>Les ingrédients sont annulés du ticket</span>
              </div>
              <div className="card-body" style={{padding:0}}>
                <table>
                  <thead><tr><th>Cocktail</th><th><span className="badge badge-am">AM</span></th><th><span className="badge badge-pm">PM</span></th><th><span className="badge badge-total">TOTAL</span></th></tr></thead>
                  <tbody>
                    {cocktailsList.map(c => {
                      ...
                      return (
                        <tr key={c.id}>
                          <td><div className="item-name">{c.name}</div>
                            <div style={{fontSize:".68rem",color:"var(--muted)"}}>
                              {(c.ingredients||[]).map(i=>{
                                const VINIER_LABELS={"vinier_blanc":"Vinier Blanc","vinier_rouge":"Vinier Rouge"};
                                const lbl=VINIER_LABELS[i.key]||[...BUG_ITEMS,...VIN_ITEMS].find(x=>(x.id||x)===i.key)?.label||AUTRES_BOISSONS.find(x=>x===i.key)||i.key;
                          ...
                          <td><input type="number" min="0" value={amVal}
                            readOnly={shift==="PM"||(shift==="AM"&&shiftReadOnly)} className={(shift==="PM"||(shift==="AM"&&shiftReadOnly))?"locked":""}
                            onChange={e=>{const v=e.target.value;setSheet(s=>({...s,cocktails:{...s.cocktails,[c.id]:{...d,am:v,total:(parseFloat(v)||0)+(parseFloat(pmVal)||0)}}}));}}/></td>
                          <td><input type="number" min="0" value={pmVal}
                            readOnly={shift==="AM"||(shift==="PM"&&shiftReadOnly)} className={(shift==="AM"||(shift==="PM"&&shiftReadOnly))?"locked":""}
                            onChange={e=>{const v=e.target.value;setSheet(s=>({...s,cocktails:{...s.cocktails,[c.id]:{...d,pm:v,total:(parseFloat(amVal)||0)+(parseFloat(v)||0)}}}));}}/></td>
                          <td className="total-cell">{total}</td>
                        </tr>
                      );
                    })}
        ...
        {(() => {
          ...
          const setHH = (val) => setSheet(s => ({ ...s, happyHour: { ...s.happyHour, [shift]: val } }));
          return (
            <div className="card">
              <div className="card-header">
                <h3>🍻 Happy Hour 5 à 7</h3>
                <span style={{marginLeft:"auto",fontSize:".72rem",color:"var(--muted)"}}>Bières à -1$</span>
              </div>
              <div className="card-body">
                <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                  <div>
                    <div style={{fontSize:".74rem",color:"var(--muted)",marginBottom:4}}>Nombre vendu — shift {shift}</div>
                    <input type="number" min="0" max={totalBieresShift} value={hhVal}
                      readOnly={shiftReadOnly} className={shiftReadOnly?"locked":""}
                      style={{width:120,borderColor:hhError?"var(--red)":undefined}}
                      onChange={e=>setHH(e.target.value)} placeholder="0" />
                  </div>
                  <div style={{fontSize:".78rem",color:"var(--muted)"}}>Total bières : <strong>{totalBieresShift}</strong></div>
                  {hhNum>0&&!hhError&&<div style={{fontSize:".78rem",color:"var(--orange)"}}>→ Déduction : <strong>-{hhNum.toFixed(2)} $</strong></div>}
                </div>
                {hhError&&<div style={{color:"var(--red)",fontSize:".78rem",marginTop:8}}>⛔ Ne peut pas dépasser {totalBieresShift}</div>}
              </div>
            </div>
          );
        })()}
        ...
        {/* ── PROMOS (si autorisé) ── */}
        {canUsePromos(employee) && (() => {
          const prices = loadPrices();
          // Promo Shooter : bugs 7, 8, 9
          const totalShooters = ["bug7","bug8","bug9"].reduce((s,k) => {
            return s + (shift==="AM" ? (sheet.bugs?.[k]?.venteAM||0) : (sheet.bugs?.[k]?.ventePM||0));
          }, 0);
          const avgShooterPrice = ["bug7","bug8","bug9"].map(k=>parseFloat(prices[k])||0).filter(p=>p>0)
            .reduce((a,b,_,arr)=>a+b/arr.length,0) || 0;
          const promoShooterVal = sheet.promoShooter?.[shift] || "";
          const promoShooterNum = parseFloat(promoShooterVal) || 0;

          // Promo Petite Bière
          const totalBieres = PETITES_BIERES.reduce((s,n)=>s+(parseFloat(sheet.regular?.[n]?.[shift.toLowerCase()])||0),0);
          const avgBierePrice = PETITES_BIERES.map(n=>parseFloat(prices[n])||0).filter(p=>p>0)
            .reduce((a,b,_,arr)=>a+b/arr.length,0) || 5;
          const promoBiereVal = sheet.promoBiere?.[shift] || "";
          const promoBiereNum = parseFloat(promoBiereVal) || 0;

          // Promo Vin
          const totalVins = VIN_ITEMS.filter(v=>v.type==="vinier").reduce((s,v)=>
            s+(shift==="AM"?(sheet.vins?.[v.id]?.venteAM||0):(sheet.vins?.[v.id]?.ventePM||0)),0);
          const vinierPrice = parseFloat(prices["vinier_blanc1"]) || 9.75;
          const promoVinVal = sheet.promoVin?.[shift] || "";
          const promoVinNum = parseFloat(promoVinVal) || 0;

          return (
            <div className="card">
              <div className="card-header">
                <h3>🏷 Promos</h3>
                <span style={{marginLeft:"auto",fontSize:".72rem",background:"rgba(168,85,247,.15)",
                  color:"#a855f7",borderRadius:5,padding:"2px 8px",fontWeight:600}}>
                  Accès restreint
                </span>
              </div>
              <div className="card-body" style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                {/* Promo Shooter */}
                <div>
                  <div style={{fontWeight:600,marginBottom:4}}>🥃 Promo Shooter — 2$
                    <span style={{fontSize:".72rem",color:"var(--muted)",marginLeft:8}}>
                      (prix normal ~{avgShooterPrice.toFixed(2)}$, déduction {Math.max(0,avgShooterPrice-2).toFixed(2)}$/shooter)
                    </span>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                    <input type="number" min="0" max={totalShooters} value={promoShooterVal}
                      readOnly={shiftReadOnly} className={shiftReadOnly?"locked":""}
                      style={{width:120,borderColor:promoShooterNum>totalShooters?"var(--red)":undefined}}
                      onChange={e=>setSheet(s=>({...s,promoShooter:{...s.promoShooter,[shift]:e.target.value}}))}
                      placeholder="0" />
                    <span style={{fontSize:".78rem",color:"var(--muted)"}}>Shooters vendus ce shift : <strong>{totalShooters}</strong></span>
                    {promoShooterNum>0&&<span style={{fontSize:".78rem",color:"var(--orange)"}}>→ -{(promoShooterNum*Math.max(0,avgShooterPrice-2)).toFixed(2)} $</span>}
                  </div>
                </div>

                {/* Promo Petite Bière */}
                <div>
                  <div style={{fontWeight:600,marginBottom:4}}>🍺 Promo Petite Bière — 4$
                    <span style={{fontSize:".72rem",color:"var(--muted)",marginLeft:8}}>
                      (déduction {Math.max(0,avgBierePrice-4).toFixed(2)}$/bière)
                    </span>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                    <input type="number" min="0" max={totalBieres} value={promoBiereVal}
                      readOnly={shiftReadOnly} className={shiftReadOnly?"locked":""}
                      style={{width:120,borderColor:promoBiereNum>totalBieres?"var(--red)":undefined}}
                      onChange={e=>setSheet(s=>({...s,promoBiere:{...s.promoBiere,[shift]:e.target.value}}))}
                      placeholder="0" />
                    <span style={{fontSize:".78rem",color:"var(--muted)"}}>Petites bières ce shift : <strong>{totalBieres}</strong></span>
                    {promoBiereNum>0&&<span style={{fontSize:".78rem",color:"var(--orange)"}}>→ -{(promoBiereNum*Math.max(0,avgBierePrice-4)).toFixed(2)} $</span>}
                  </div>
                </div>

                {/* Promo Vin */}
                <div>
                  <div style={{fontWeight:600,marginBottom:4}}>🍷 Promo Vin — 5$
                    <span style={{fontSize:".72rem",color:"var(--muted)",marginLeft:8}}>
                      (déduction {Math.max(0,vinierPrice-5).toFixed(2)}$/verre)
                    </span>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                    <input type="number" min="0" max={totalVins} value={promoVinVal}
                      readOnly={shiftReadOnly} className={shiftReadOnly?"locked":""}
                      style={{width:120,borderColor:promoVinNum>totalVins?"var(--red)":undefined}}
                      onChange={e=>setSheet(s=>({...s,promoVin:{...s.promoVin,[shift]:e.target.value}}))}
                      placeholder="0" />
                    <span style={{fontSize:".78rem",color:"var(--muted)"}}>Verres vinier ce shift : <strong>{totalVins}</strong></span>
                    {promoVinNum>0&&<span style={{fontSize:".78rem",color:"var(--orange)"}}>→ -{(promoVinNum*Math.max(0,vinierPrice-5)).toFixed(2)} $</span>}
                  </div>
                </div>
              </div>
            </div>
        ...
        {(() => {
          const prices = loadPrices();
          const offertes = (sheet.offertes || []).filter(o => o.shift === shift);
          const ALL_ITEMS = [
            ...PETITES_BIERES.map(n=>({id:n,label:n,cat:"🍺"})),
            ...GROSSES_BIERES.map(n=>({id:n,label:n,cat:"🍺"})),
            ...BUG_ITEMS.map(b=>({id:b.id,label:b.label,cat:"🥃"})),
            ...VIN_ITEMS.map(v=>({id:v.id,label:v.label,cat:"🍷"})),
            ...AUTRES_BOISSONS.map(n=>({id:n,label:n,cat:"🥤"})),
            ...NOURRITURE.map(n=>({id:n,label:n,cat:"🍗"})),
          ];

          const addOfferte = () => {
            if (!newOfferte.item) return;
            const found = ALL_ITEMS.find(i=>i.id===newOfferte.item);
            if (!found) return;
            const prix = (parseFloat(prices[newOfferte.item])||0) * (parseFloat(newOfferte.qty)||1);
            const entry = {
              id: Date.now().toString(),
              shift, item: newOfferte.item,
              label: found.label, cat: found.cat,
              qty: parseFloat(newOfferte.qty)||1,
              prix, note: newOfferte.note,
              approuve: null,
              ts: new Date().toLocaleString("fr-CA"),
            };
            setSheet(s=>({...s, offertes:[...(s.offertes||[]), entry]}));
            setNewOfferte({item:"",qty:1,note:""});
            setShowOfferteForm(false);
          };

          const removeOfferte = (id) => {
            setSheet(s=>({...s, offertes:(s.offertes||[]).filter(o=>o.id!==id)}));
          };

          const totalOffertes = offertes.filter(o=>o.approuve!==false).reduce((s,o)=>s+(parseFloat(o.prix)||0),0);

          return (
            <div className="card">
              <div className="card-header">
                <h3>🎁 Offert par Mathieu</h3>
                <span style={{marginLeft:"auto",fontSize:".72rem",color:"var(--muted)"}}>
                  Déduit du total caisse si approuvé
                </span>
              </div>
              <div className="card-body">
                {offertes.length === 0 && !showForm && (
                  <div style={{color:"var(--muted)",fontSize:".82rem",marginBottom:8}}>
                    Aucune consommation offerte ce shift.
                  </div>
                )}
                {offertes.length > 0 && (
                  <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:10}}>
                    {offertes.map(o => (
                      <div key={o.id} style={{display:"flex",alignItems:"center",gap:8,
                        background:"var(--surface)",borderRadius:7,padding:"8px 10px",
                        border:`1px solid ${o.approuve===true?"var(--green)":o.approuve===false?"var(--red)":"var(--border)"}`}}>
                        <span style={{fontSize:".82rem",flex:1}}>
                          {o.cat} {o.label} × {o.qty}
                          {o.note && <span style={{color:"var(--muted)",fontSize:".72rem"}}> — {o.note}</span>}
                        </span>
                        <span style={{fontSize:".82rem",fontWeight:600,
                          color:o.approuve===false?"var(--red)":"var(--orange)"}}>
                          {o.approuve===false?"[Refusé]":o.approuve===true?"[✓]":"[En attente]"}
                          {" "}-{(parseFloat(o.prix)||0).toFixed(2)} $
                        </span>
                        {!shiftReadOnly && o.approuve===null && (
                          <button style={{background:"none",border:"none",color:"var(--red)",cursor:"pointer",fontSize:".9rem"}}
                            onClick={()=>removeOfferte(o.id)}>✕</button>
                        )}
                      </div>
                    ))}
                    {totalOffertes > 0 && (
                      <div style={{fontSize:".78rem",color:"var(--orange)",textAlign:"right"}}>
                        Total déduit : -{totalOffertes.toFixed(2)} $
                      </div>
                    )}
                  </div>
                )}
                {!shiftReadOnly && (
                  showOfferteForm ? (
                    <div style={{display:"flex",flexDirection:"column",gap:8,
                      background:"var(--surface)",borderRadius:7,padding:10,
                      border:"1px solid var(--border)"}}>
                      <select value={newOfferte.item} style={{width:"100%"}}
                        onChange={e=>setNewOfferte(f=>({...f,item:e.target.value}))}>
                        <option value="">Choisir un article...</option>
                        {["🍺 Petites Bières","🍺 Grosses Bières","🥃 Forts","🍷 Vins","🥤 Boissons","🍗 Nourriture"].map((cat,ci) => {
                          const lists = [PETITES_BIERES,GROSSES_BIERES,BUG_ITEMS,VIN_ITEMS,AUTRES_BOISSONS,NOURRITURE];
                          const items = lists[ci];
                          return (
                            <optgroup key={cat} label={cat}>
                              {items.map(i=>{
                                const id = i.id||i; const label = i.label||i;
                      ...
                      <div style={{display:"flex",gap:8}}>
                        <div style={{flex:1}}>
                          <div style={{fontSize:".72rem",color:"var(--muted)",marginBottom:2}}>Quantité</div>
                          <input type="number" min="1" step="1" value={newOfferte.qty} style={{width:"100%"}}
                            onChange={e=>setNewOfferte(f=>({...f,qty:e.target.value}))} />
                        </div>
                        <div style={{flex:2}}>
                          <div style={{fontSize:".72rem",color:"var(--muted)",marginBottom:2}}>Note (optionnel)</div>
                          <input type="text" value={newOfferte.note} placeholder="Ex: table 5, ami de..."
                            style={{width:"100%"}}
                            onChange={e=>setNewOfferte(f=>({...f,note:e.target.value}))} />
                        </div>
                      </div>
                      <div style={{display:"flex",gap:8}}>
                        <button className="btn btn-accent" onClick={addOfferte}>➕ Ajouter</button>
                        <button className="btn btn-ghost" onClick={()=>setShowOfferteForm(false)}>Annuler</button>
                      </div>
                    </div>
                  ) : (
                    <button className="btn btn-ghost" style={{fontSize:".8rem"}}
                      onClick={()=>setShowOfferteForm(true)}>
                      + Ajouter une consommation offerte
                    </button>
                  )
                )}
              </div>
        ...
        {/* ── BOUTEILLE GLOBALE — avant Commentaire ── */}
        <div className="card">
        ...
        {/* ── ALERTE ERREUR ── */}
        {(() => {
          const alertes = (sheet.alertesErreur || []).filter(a => a.shift === shift);

          const addAlerte = () => {
            if (!errText.trim()) return;
            const alerte = {
              id: Date.now().toString(),
              shift, texte: errText.trim(),
              soumisePar: employee?.name || "?",
              ts: new Date().toLocaleString("fr-CA"),
              statut: null, // null=en attente, true=accepté, false=refusé
            };
            setSheet(s=>({...s, alertesErreur:[...(s.alertesErreur||[]), alerte]}));
            setErrText("");
            setShowErrForm(false);
            showToast("⚠️ Alerte envoyée au gérant", "warn");
          };

          return (
            <div className="card" style={{borderColor:"var(--orange)",borderWidth:1}}>
              <div className="card-header">
                <h3>⚠️ Signaler une erreur</h3>
                <span style={{fontSize:".72rem",color:"var(--muted)",marginLeft:"auto"}}>
                  Visible par le gérant seulement
                </span>
              </div>
              <div className="card-body">
                {alertes.length > 0 && (
                  <div style={{marginBottom:10,display:"flex",flexDirection:"column",gap:6}}>
                    {alertes.map(a=>(
                      <div key={a.id} style={{padding:"8px 10px",borderRadius:7,fontSize:".82rem",
                        background:a.statut===true?"rgba(34,197,94,.1)":a.statut===false?"rgba(239,68,68,.1)":"rgba(249,115,22,.1)",
                        border:`1px solid ${a.statut===true?"var(--green)":a.statut===false?"var(--red)":"var(--orange)"}`}}>
                        <div style={{fontWeight:600,marginBottom:2}}>
                          {a.statut===true?"✅ Acceptée":a.statut===false?"❌ Refusée":"⏳ En attente"}
                        </div>
                        <div>{a.texte}</div>
                        <div style={{fontSize:".7rem",color:"var(--muted)",marginTop:2}}>{a.ts}</div>
                      </div>
                    ))}
                  </div>
                )}
                {!showErrForm ? (
                  <button className="btn btn-ghost" style={{borderColor:"var(--orange)",color:"var(--orange)"}}
                    onClick={()=>setShowErrForm(true)}>
                    ⚠️ Signaler une erreur dans ma feuille
                  </button>
                ) : (
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    <textarea rows={3} value={errText} placeholder="Décris l'erreur (ex: j'ai mis 45 au lieu de 54 pour le Fort Sortilège AM)..."
                      style={{resize:"vertical"}} onChange={e=>setErrText(e.target.value)} autoFocus />
                    <div style={{display:"flex",gap:8}}>
                      <button className="btn btn-accent" onClick={addAlerte}>Envoyer au gérant</button>
                      <button className="btn btn-ghost" onClick={()=>setShowErrForm(false)}>Annuler</button>
                    </div>
                  </div>
                )}
              </div>
...
function ManagerView({ sheet, setSheet, prevFinsPM, inventory, setInventory, history, setHistory, onLogout, showToast, currentEmployee }) {
  const [tab, setTab] = useState("today");
  const isGerantRole = isGerant(currentEmployee) || !currentEmployee; // no employee = gérant principal
  ...
  const applyDeduction = () => {
    ...
    setInventory(newInv);
    // Save this sheet individually as sheet_DATE for history reconstruction
    save("sheet_"+sheet.date, sheet);
    setHistory(h => {
      const idx = h.findIndex(d => d.date === sheet.date);
      if (idx >= 0) { const hh=[...h]; hh[idx]=sheet; return hh; }
      return [sheet, ...h].sort((a,b) => b.date.localeCompare(a.date));
    });
    showToast("✅ Inventaire mis à jour et journée archivée!","ok");
  };

  // Tabs gérant = tout, manageuse = accès limité
  const allTabs = [
    { id:"today",       label:"📋 Feuille du jour",  gerantOnly: false },
    { id:"anomalies",   label:"⚠ Anomalies",          gerantOnly: false, count: anomalies.length },
    { id:"inventory",   label:"📦 Inventaire",         gerantOnly: false },
    { id:"commandes",   label:"📥 Commandes",          gerantOnly: false },
    { id:"depots",      label:"💰 Dépôts",             gerantOnly: true  },
    { id:"coffre",      label:"🔒 Coffre",              gerantOnly: true  },
    { id:"semaine",     label:"📊 Semaine",            gerantOnly: true  },
    { id:"analytique",  label:"📈 Analytique",         gerantOnly: true  },
    { id:"produits",    label:"🏷 Produits",           gerantOnly: true  },
    { id:"cocktails",   label:"🍹 Cocktails",          gerantOnly: true  },
    { id:"caisse",      label:"🧾 Caisse",             gerantOnly: true  },
    { id:"verifcaisse", label:"✅ Vérif. Caisse",      gerantOnly: true  },
    { id:"employes",    label:"👥 Employés",           gerantOnly: true  },
    { id:"loto",        label:"🎰 Loto-Québec",        gerantOnly: true  },
    { id:"history",     label:"🗂 Historique",         gerantOnly: true  },
  ];
  const tabs = allTabs.filter(t => isGerantRole || !t.gerantOnly);
  ...
  return (
    <div className="app">
      <div className="topbar">
        ...
        <button className="btn btn-ghost" title="Exporter toutes les données"
          onClick={() => {
            const data = {};
            ...
            a.href = url; a.download = `club-des-ex-${new Date().toISOString().slice(0,10)}.json`;
            a.click(); URL.revokeObjectURL(url);
            showToast("📤 Données exportées!", "ok");
          }}>📤</button>
        <label className="btn btn-ghost" title="Importer des données" style={{cursor:"pointer"}}>
          📥
            ...
            onChange={e => {
              ...
              reader.onload = ev => {
                try {
                  ...
                  showToast("📥 Importé — rechargement...", "ok");
                  setTimeout(() => window.location.reload(), 1200);
                } catch { showToast("⛔ Fichier invalide", "err"); }
              };
              reader.readAsText(file); e.target.value="";
            }} />
        </label>
        <button className="btn btn-ghost" title="Pousser toutes les données vers Firebase (synchro)"
          onClick={async () => {
            showToast("⏳ Envoi vers Firebase...", "ok");
            const keys = Object.keys(localStorage).filter(k => !k.startsWith("__fb") && k !== "app_unlocked" && k !== "mgr_unlocked");
            let count = 0;
            ...
            showToast(`☁️ ${count} éléments envoyés à Firebase!`, "ok");
          }}>☁️ Sync</button>
      </div>
      <div className="nav">
        {tabs.map(t=>(
      ...
      <div className="main">
        {tab==="today"      && <ManagerToday sheet={sheet} setSheet={setSheet} prevFinsPM={prevFinsPM} onApply={applyDeduction} anomalies={anomalies} showToast={showToast} />}
        ...
        {tab==="depots"     && <ManagerDepots today={TODAY()} />}
        {tab==="coffre"     && <ManagerCoffre today={TODAY()} history={history} />}
        ...
        {tab==="loto"       && <ManagerLoto today={TODAY()} history={history} />}
...
const calcVenteShift = (sheet, shift, prices) => {
  ...
  // Promos — déduit la différence (prix normal - prix promo)
  // Promo Shooter : bugs 7, 8, 9 vendus à 2$ au lieu du prix normal
  const promoShooterQty = parseFloat(sheet.promoShooter?.[shift]) || 0;
  if (promoShooterQty > 0) {
    const avgShooterPrice = [
      parseFloat(prices?.["bug7"])||0,
      parseFloat(prices?.["bug8"])||0,
      parseFloat(prices?.["bug9"])||0,
    ].filter(p=>p>0).reduce((a,b,_,arr)=>a+b/arr.length,0) || parseFloat(prices?.["bug7"])||0;
    total -= promoShooterQty * Math.max(0, avgShooterPrice - 2);
  }
  // Promo Petite Bière : petites bières vendues à 4$
  const promoBiereQty = parseFloat(sheet.promoBiere?.[shift]) || 0;
  if (promoBiereQty > 0) {
    const avgBierePrice = PETITES_BIERES.map(n=>parseFloat(prices?.[n])||0)
      .filter(p=>p>0).reduce((a,b,_,arr)=>a+b/arr.length,0) || 5;
    total -= promoBiereQty * Math.max(0, avgBierePrice - 4);
  }
  // Promo Vin : verres de vinier vendus à 5$
  const promoVinQty = parseFloat(sheet.promoVin?.[shift]) || 0;
  if (promoVinQty > 0) {
    const vinierPrice = parseFloat(prices?.["vinier_blanc1"]) || 9.75;
    total -= promoVinQty * Math.max(0, vinierPrice - 5);
  }
  ...
  // Consommations offertes — déduites si approuvées ou en attente (pas si refusées)
  const offertes = sheet.offertes || [];
  offertes.forEach(o => {
    if (o.shift === shift && o.approuve !== false) {
      total -= parseFloat(o.prix) || 0;
    }
  });
...
function EmployeeCaisse({ sheet, setSheet, shift, employee, showToast, prevCaisseTotal }) {
  ...
  const [editMode, setEditMode] = useState(false);
  const [showNipModal, setShowNipModal] = useState(false); // NIP pour soumettre
  const [showUnlockModal, setShowUnlockModal] = useState(false); // NIP gérant pour modifier
  const [nipInput, setNipInput] = useState("");
  const [nipErr, setNipErr] = useState(false);

  const isSubmitted = !!c.soumiseTs;
  const isLocked = isSubmitted && !editMode;
  // La caisse appartient à la personne qui l'a soumise
  const caisseOwner = c.soumisePar || "";
  const isOwner = !caisseOwner || caisseOwner === employee?.name;
  ...
  // Auto-fill argentRecu depuis le shift précédent — se recalcule si prevCaisseTotal change
  useEffect(() => {
    if (prevCaisseTotal === "" || prevCaisseTotal === undefined || prevCaisseTotal === 0) return;
    // Remplit argentRecu si vide OU si la valeur a changé (ex: après Firebase sync)
    const currentRecu = parseFloat(c.argentRecu) || 0;
    const expectedRecu = parseFloat(prevCaisseTotal) || 0;
    if (Math.abs(currentRecu - expectedRecu) > 0.001) {
      setSheet(s => {
        ...
        const recu     = parseFloat(prevCaisseTotal)       || 0;
        ...
        const vente    = calcVenteShift(s, shift, loadPrices());
        caisse[shift].venteCalculee = vente;
        caisse[shift].totalCaisse = Math.round((recu + machines + coffre - coupons - factures + vente)*100)/100;
        return { ...s, caisse };
      });
    }
  }, [prevCaisseTotal, shift]);
  ...
  const doSubmit = (nip) => {
    // Vérifie que c'est bien le NIP de cet employé
    const employees = loadEmployees();
    const emp = employees.find(e => e.name === employee?.name);
    const validNip = emp && emp.pin === nip;
    if (!validNip) {
      setNipErr(true);
      setTimeout(() => { setNipInput(""); setNipErr(false); }, 700);
      return;
    }
    setSheet(s => {
      const caisse = { ...s.caisse };
    ...
    setNipInput("");
    showToast("✅ Caisse soumise et verrouillée!", "ok");
  };

  const handleSubmit = () => setShowNipModal(true);

  const handleReopen = () => setShowUnlockModal(true);

  const handleNipKey = (k, mode) => {
    if (k === "DEL") { setNipInput(p => p.slice(0,-1)); setNipErr(false); return; }
    const np = nipInput + k;
    setNipInput(np);
    if (np.length === 4) {
      if (mode === "submit") {
        ...
        // Mode unlock — gérant seulement
        const ok = checkManagerPin(np);
        if (ok) {
          setEditMode(true);
          setShowUnlockModal(false);
          setNipInput("");
          showToast("🔓 Caisse déverrouillée par le gérant", "ok");
        } else {
          setNipErr(true);
          setTimeout(() => { setNipInput(""); setNipErr(false); }, 700);
        }
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <div className="card-body">
          {/* Argent reçu — auto mais modifiable par le gérant si besoin */}
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
            padding:"10px 0",borderBottom:"1px solid var(--border)"}}>
            <div>
              <div style={{fontSize:".88rem",fontWeight:500}}>💵 Argent reçu</div>
              <div style={{fontSize:".72rem",color:"var(--muted)"}}>
                {shift==="PM"
                  ? `Total caisse AM${sheet.caisse?.AM?.totalCaisse ? ` — ${parseFloat(sheet.caisse.AM.totalCaisse).toFixed(2)} $` : " — non disponible"}`
                  : `Total caisse PM de la veille${prevCaisseTotal ? ` — ${parseFloat(prevCaisseTotal).toFixed(2)} $` : " — non disponible"}`}
              </div>
              {isManagerUnlocked() && (
                <div style={{fontSize:".68rem",color:"var(--accent)"}}>🔓 Modifiable par le gérant</div>
              )}
            </div>
            <input type="number" value={c.argentRecu||""}
              readOnly={!isManagerUnlocked()}
              className={!isManagerUnlocked() ? "locked" : ""}
              style={{textAlign:"right",width:130,
                borderColor:isManagerUnlocked()?"var(--accent)":undefined}}
              onChange={e=>setSheet(s=>{
                const caisse={...s.caisse};
                caisse[shift]={...caisse[shift],argentRecu:e.target.value};
                return {...s,caisse};
              })} />
          </div>
      ...
      <div className="btn-row">
        {!isSubmitted || editMode ? (
          ...
          <div style={{color:"var(--green)",fontSize:".82rem"}}>
            ✅ Caisse soumise par <strong>{caisseOwner}</strong> — verrouillée
          </div>
        )}
        {editMode && (
      ...
      {/* Modal NIP — soumission */}
      {showNipModal && (
        <div className="pin-modal-overlay" onClick={()=>{setShowNipModal(false);setNipInput("");}}>
          <div className="pin-modal" onClick={e=>e.stopPropagation()}>
            <h3>🔐 Confirme avec ton NIP</h3>
            <div className="sub">Entre ton NIP personnel pour verrouiller ta caisse</div>
            <div className="pin-modal-display" style={{color:nipErr?"var(--red)":"var(--text)"}}>
              {nipInput.replace(/./g,"●")||"_ _ _ _"}
            </div>
            <div className="pin-modal-grid">
              {["1","2","3","4","5","6","7","8","9","✕","0","DEL"].map(k=>(
                <button key={k} className="pin-key"
                  onClick={()=>{
                    if (k==="✕") { setShowNipModal(false); setNipInput(""); }
                    else handleNipKey(k==="DEL"?"DEL":k, "submit");
                  }}
                  style={k==="✕"?{color:"var(--red)"}:{}}>
                  {k==="DEL"?"⌫":k}
                </button>
              ))}
            </div>
            {nipErr && <div style={{color:"var(--red)",fontSize:".78rem",marginTop:8}}>⛔ NIP incorrect</div>}
          </div>
        </div>
      )}

      {/* Modal NIP — déverrouillage gérant */}
      {showUnlockModal && (
        <div className="pin-modal-overlay" onClick={()=>{setShowUnlockModal(false);setNipInput("");}}>
          <div className="pin-modal" onClick={e=>e.stopPropagation()}>
            <h3>🔐 Code Gérant requis</h3>
            <div className="sub">Seul le gérant peut modifier une caisse soumise</div>
            <div className="pin-modal-display" style={{color:nipErr?"var(--red)":"var(--text)"}}>
              {nipInput.replace(/./g,"●")||"_ _ _ _"}
            </div>
            <div className="pin-modal-grid">
              {["1","2","3","4","5","6","7","8","9","✕","0","DEL"].map(k=>(
                <button key={k} className="pin-key"
                  onClick={()=>{
                    if (k==="✕") { setShowUnlockModal(false); setNipInput(""); }
                    else handleNipKey(k==="DEL"?"DEL":k, "unlock");
                  }}
                  style={k==="✕"?{color:"var(--red)"}:{}}>
                  {k==="DEL"?"⌫":k}
                </button>
            ...
            {nipErr && <div style={{color:"var(--red)",fontSize:".78rem",marginTop:8}}>⛔ Code incorrect</div>}
          </div>
        </div>
      )}
    </>
...
function ManagerVerifCaisse({ sheet, setSheet, history, setHistory, showToast }) {
  ...
  const loadDate = (date) => {
    setViewDate(date);
    setEditMode(false);
    setPinUnlocked(date === TODAY() || isManagerUnlocked());
    setPinInput("");
    setPinErr(false);
  };
...
// ─── MANAGER COFFRE ──────────────────────────────────────────────────────────
// Suivi du coffre jour par jour
// Formule : Coffre = Coffre veille + Dépôt coffre (du jour) - Argent sorti AM - Argent sorti PM

function ManagerCoffre({ today, history }) {
  ...
  const fmtFull = (ds) => {
    ...
    return `${jours[d.getDay()]} ${d.getDate()} ${monthNames[d.getMonth()]}`;
  };

  const getDates = () => viewMode === "semaine"
    ? Array.from({length:7}, (_,i) => addDays(selectedWeek, i))
    : Array.from({length:30}, (_,i) => addDays(today, -29+i));

  const dates = getDates();

  // Build histMap from history + localStorage sheets
  const histMap = {};
  history.forEach(d => { if(d?.date) histMap[d.date] = d; });
  dates.forEach(date => {
    if (!histMap[date]) {
      const s = load("sheet_"+date, null);
      if (s) histMap[date] = migrateSheet(s);
    }
  });

  // Calculate coffre chain — part du solde du jour précédent la période
  const rows = [];

  // Trouver le solde cumulatif jusqu'au jour avant la période
  const firstDate = dates[0];
  const dayBefore = addDays(firstDate, -1);
  let runningCoffre = 0;

  // Reconstruire le coffre depuis le début des données jusqu'au jour avant
  const allDatesBeforePeriod = [];
  for (let i = 0; i < 365; i++) {
    const d = addDays(firstDate, -(365 - i));
    allDatesBeforePeriod.push(d);
  }
  allDatesBeforePeriod.forEach(date => {
    if (date >= firstDate) return;
    const depot = loadDepot(date);
    const sheet = histMap[date] || (()=>{const s=load("sheet_"+date,null);return s?migrateSheet(s):null;})();
    const dc  = parseFloat(depot.coffre)||0;
    const sAM = parseFloat(sheet?.caisse?.AM?.argentCoffre)||0;
    const sPM = parseFloat(sheet?.caisse?.PM?.argentCoffre)||0;
    if (dc > 0 || sAM > 0 || sPM > 0) {
      runningCoffre = runningCoffre + dc - sAM - sPM;
    }
  });

  const soldeDepart = runningCoffre;

  dates.forEach(date => {
    const depot  = loadDepot(date);
    const sheet  = histMap[date] || (()=>{const s=load("sheet_"+date,null);return s?migrateSheet(s):null;})();
    const depotCoffre = parseFloat(depot.coffre) || 0;
    const sortieAM    = parseFloat(sheet?.caisse?.AM?.argentCoffre) || 0;
    const sortiePM    = parseFloat(sheet?.caisse?.PM?.argentCoffre) || 0;
    const totalSortie = sortieAM + sortiePM;

    runningCoffre = runningCoffre + depotCoffre - totalSortie;

    rows.push({
      date, fmtDate: fmtFull(date),
      depotCoffre, sortieAM, sortiePM, totalSortie,
      soldeDebut: runningCoffre - depotCoffre + totalSortie,
      coffreCalc: runningCoffre,
      isToday: date === today,
      hasData: depotCoffre > 0 || sortieAM > 0 || sortiePM > 0,
    });
  });
  ...
  return (
    <>
      <div className="card">
        <div className="card-header">
          <h3>🔒 Suivi du Coffre</h3>
        </div>
        <div className="card-body">
          <div style={{fontSize:".8rem",color:"var(--muted)",marginBottom:12,lineHeight:1.6}}>
            <strong>Formule :</strong> Coffre = Coffre veille + Dépôt coffre − Argent sorti AM − Argent sorti PM
          </div>
          <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
            {[{v:"semaine",l:"Semaine"},{v:"30jours",l:"30 derniers jours"}].map(p=>(
              <button key={p.v} className={`btn ${viewMode===p.v?"btn-accent":"btn-ghost"}`}
                onClick={()=>setViewMode(p.v)}>{p.l}</button>
            ))}
          </div>
          {viewMode === "semaine" && (
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
              <button className="btn btn-ghost" style={{padding:"4px 12px"}}
                onClick={()=>setSelectedWeek(w=>addDays(w,-7))}>‹</button>
              <span style={{fontWeight:600}}>
                {fmtFull(selectedWeek)} → {fmtFull(addDays(selectedWeek,6))}
              </span>
              <button className="btn btn-ghost" style={{padding:"4px 12px"}}
                onClick={()=>setSelectedWeek(w=>addDays(w,7))}>›</button>
            </div>
        ...
        <div className="card-body" style={{padding:0}}>
          <table>
            <thead>
              <tr>
                <th>Date</th>
                <th style={{textAlign:"right"}}>Solde début</th>
                <th style={{textAlign:"right"}}>+ Dépôt coffre</th>
                <th style={{textAlign:"right"}}>− Sorti AM</th>
                <th style={{textAlign:"right"}}>− Sorti PM</th>
                <th style={{textAlign:"right"}}>= Fin journée</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.date} style={{
                  background: r.isToday ? "rgba(232,160,32,.06)" : !r.hasData ? "rgba(0,0,0,.02)" : undefined,
                  opacity: !r.hasData ? 0.5 : 1,
                }}>
                  <td style={{fontSize:".82rem",fontWeight:r.isToday?700:400}}>
                    {r.fmtDate}
                    {r.isToday && <span style={{fontSize:".65rem",color:"var(--accent)",marginLeft:6}}>aujourd'hui</span>}
                  </td>
                  <td style={{textAlign:"right",color:"var(--muted)",fontSize:".82rem"}}>
                    {r.soldeDebut !== 0 ? `${r.soldeDebut.toFixed(2)} $` : "—"}
                  </td>
                  <td style={{textAlign:"right",color:"var(--green)",fontSize:".85rem"}}>
                    {r.depotCoffre > 0 ? `+${r.depotCoffre.toFixed(2)} $` : "—"}
                  </td>
                  <td style={{textAlign:"right",color:r.sortieAM>0?"var(--orange)":"var(--muted)",fontSize:".85rem"}}>
                    {r.sortieAM > 0 ? `-${r.sortieAM.toFixed(2)} $` : "—"}
                  </td>
                  <td style={{textAlign:"right",color:r.sortiePM>0?"var(--orange)":"var(--muted)",fontSize:".85rem"}}>
                    {r.sortiePM > 0 ? `-${r.sortiePM.toFixed(2)} $` : "—"}
                  </td>
                  <td style={{textAlign:"right",fontWeight:700,
                    color: r.coffreCalc > 0 ? "var(--accent)" : r.coffreCalc < 0 ? "var(--red)" : "var(--muted)"}}>
                    {r.hasData || r.soldeDebut !== 0 ? `${r.coffreCalc.toFixed(2)} $` : "—"}
                  </td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr style={{borderTop:"2px solid var(--border)",background:"var(--surface)"}}>
                <td style={{fontWeight:700,padding:"10px 16px",fontSize:".82rem"}}>
                  Coffre actuel estimé
                </td>
                <td colSpan={4}/>
                <td style={{textAlign:"right",fontWeight:700,fontSize:"1.1rem",
                  color:"var(--accent)",paddingRight:16}}>
                  {rows[rows.length-1]?.coffreCalc.toFixed(2)} $
                </td>
              </tr>
            </tfoot>
          </table>
...
function ManagerDepots({ today }) {
  const [viewMode, setViewMode] = useState("semaine"); // semaine | mois | tous
  const [selectedWeek, setSelectedWeek] = useState(() => {
  ...
  const getDates = () => {
    if (viewMode === "semaine") {
      ...
      return Array.from({length:lastDay}, (_,i) => `${selectedMonth}-${String(i+1).padStart(2,"0")}`);
    } else {
      // Tous — 90 derniers jours
      return Array.from({length:90}, (_,i) => addDays(today, -89+i));
    }
  };

  const dates = getDates();

  // Charger les dépôts pour ces dates
  const depots = dates.map(date => ({
    date,
    ...loadDepot(date),
  })).filter(d => d.soumisTs || d.lotoQuebec); // garder seulement ceux qui ont des données

  // Totaux
  const totalLoto    = depots.reduce((s,d) => s + (parseFloat(d.lotoQuebec)||0), 0);
  const totalCoffre  = depots.reduce((s,d) => s + (parseFloat(d.coffre)||0), 0);
  const totalGuichet = depots.reduce((s,d) => s + (parseFloat(d.guichet)||0), 0);
  const totalCaisse  = depots.reduce((s,d) => s + (parseFloat(d.caisse)||0), 0);

  const weekLabel = () => `${fmtDate(selectedWeek)} → ${fmtDate(addDays(selectedWeek,6))}`;
  const prevWeek  = () => setSelectedWeek(w => addDays(w,-7));
  const nextWeek  = () => setSelectedWeek(w => addDays(w,7));
    ...
    <>
        ...
        <div className="card-body">
          ...
          {viewMode === "semaine" && (
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <span style={{fontWeight:600,minWidth:200,textAlign:"center"}}>{weekLabel()}</span>
              <button className="btn btn-ghost" style={{padding:"4px 12px"}}
                onClick={nextWeek} disabled={selectedWeek >= getMonday(today)}>›</button>
            </div>
          )}
          {viewMode === "mois" && (
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <button className="btn btn-ghost" style={{padding:"4px 12px"}}
                onClick={()=>setSelectedMonth(m=>{const d=new Date(m+"-01");d.setMonth(d.getMonth()-1);return d.toISOString().slice(0,7);})}>‹</button>
              <span style={{fontWeight:600,minWidth:120,textAlign:"center"}}>
                {monthNames[parseInt(selectedMonth.slice(5,7))-1]} {selectedMonth.slice(0,4)}
              </span>
              <button className="btn btn-ghost" style={{padding:"4px 12px"}}
                onClick={()=>setSelectedMonth(m=>{const d=new Date(m+"-01");d.setMonth(d.getMonth()+1);return d.toISOString().slice(0,7);})}
                disabled={selectedMonth >= today.slice(0,7)}>›</button>
            </div>
          )}
        </div>
      </div>
      ...
      {depots.length > 0 && (
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:10,marginBottom:14}}>
          {[
            {label:"🎰 Total Machines",  val:totalLoto,    color:"var(--accent)"},
            {label:"🔒 Total Coffre",    val:totalCoffre,  color:"var(--text)"},
            {label:"🏧 Total Guichet",   val:totalGuichet, color:"var(--text)"},
            {label:"💵 Total Caisse AM", val:totalCaisse,  color:"var(--green)"},
          ].map(({label,val,color})=>(
            <div key={label} style={{background:"var(--card)",border:"1px solid var(--border)",
              ...
              <div style={{fontFamily:"var(--font-head)",fontSize:"1.3rem",color}}>
                {val.toFixed(2)} $
              </div>
            </div>
          ))}
      ...
      {depots.length === 0 ? (
        <div style={{textAlign:"center",color:"var(--muted)",padding:40}}>
          Aucun dépôt enregistré pour cette période.
        </div>
      ) : (
        <div className="card">
          <div className="card-body" style={{padding:0}}>
            <table>
              <thead>
                <tr>
                  ...
                  <th style={{textAlign:"right"}}>🔒 Coffre</th>
                  <th style={{textAlign:"right"}}>🏧 Guichet</th>
                  <th style={{textAlign:"right"}}>💵 Caisse</th>
                  <th style={{textAlign:"center"}}>Statut</th>
                </tr>
              </thead>
              <tbody>
                {depots.map(d => {
                  const loto    = parseFloat(d.lotoQuebec)||0;
                  const coffre  = parseFloat(d.coffre)||0;
                  const guichet = parseFloat(d.guichet)||0;
                  const caisse  = parseFloat(d.caisse)||0;
                  const sum     = coffre + guichet + caisse;
                  const balanced = loto > 0 && Math.abs(sum - loto) < 0.01;
                  const isToday = d.date === today;
                  return (
                    <tr key={d.date} style={{
                      background: isToday ? "rgba(232,160,32,.04)" : undefined,
                    }}>
                      <td style={{fontWeight: isToday?700:400, fontSize:".85rem"}}>
                        {fmtFull(d.date)}
                        {isToday && <span style={{fontSize:".68rem",color:"var(--accent)",marginLeft:6}}>aujourd'hui</span>}
                      </td>
                      <td style={{textAlign:"right",fontWeight:600,color:"var(--accent)"}}>
                        {loto>0 ? `${loto.toFixed(2)} $` : "—"}
                      </td>
                      <td style={{textAlign:"right"}}>{coffre>0?`${coffre.toFixed(2)} $`:"—"}</td>
                      <td style={{textAlign:"right"}}>{guichet>0?`${guichet.toFixed(2)} $`:"—"}</td>
                      <td style={{textAlign:"right",color:"var(--green)",fontWeight:600}}>
                        {caisse>0?`${caisse.toFixed(2)} $`:"—"}
                      </td>
                      <td style={{textAlign:"center"}}>
                        {d.soumisTs ? (
                          <span style={{fontSize:".72rem",color:balanced?"var(--green)":"var(--orange)",
                            background:balanced?"rgba(34,197,94,.1)":"rgba(249,115,22,.1)",
                            borderRadius:5,padding:"2px 7px"}}>
                            {balanced ? "✓ Balancé" : "⚠ Écart"}
                          </span>
                        ) : (
                          <span style={{fontSize:".72rem",color:"var(--muted)"}}>Non soumis</span>
                        )}
                      </td>
                    </tr>
                  );
              ...
              <tfoot>
                <tr style={{borderTop:"2px solid var(--border)",background:"var(--surface)"}}>
                  <td style={{fontWeight:700,padding:"10px 16px",fontSize:".82rem"}}>TOTAL ({depots.length} jour{depots.length>1?"s":""})</td>
                  <td style={{textAlign:"right",fontWeight:700,color:"var(--accent)"}}>{totalLoto.toFixed(2)} $</td>
                  <td style={{textAlign:"right",fontWeight:700}}>{totalCoffre.toFixed(2)} $</td>
                  <td style={{textAlign:"right",fontWeight:700}}>{totalGuichet.toFixed(2)} $</td>
                  <td style={{textAlign:"right",fontWeight:700,color:"var(--green)"}}>{totalCaisse.toFixed(2)} $</td>
                  <td></td>
                </tr>
              </tfoot>
            </table>
...
function ManagerProduits({ showToast, inventory, setInventory }) {
  ...
  const [subTab, setSubTab] = useState("catalogue"); // catalogue | prix | ajouter
  const [form, setForm] = useState({ name:"", category:"petites_bieres", price:"" });
  const [editId, setEditId] = useState(null);

  // Catégories avec leur clé de stockage
  const CATEGORIES = [
    { value:"petites_bieres", label:"🍺 Petites Bières"   },
    { value:"grosses_bieres", label:"🍺 Grosses Bières"   },
    { value:"autres_bieres",  label:"🍻 Autres Bières"    },
    { value:"boissons",       label:"🥤 Autres Boissons"  },
  ...
  // Tous les produits (standards + customs) groupés par catégorie
  const allProductsByCategory = () => {
    const groups = {
      petites_bieres: PETITES_BIERES.map(n => ({ id:n, name:n, builtin:true })),
      grosses_bieres: GROSSES_BIERES.map(n => ({ id:n, name:n, builtin:true })),
      autres_bieres:  AUTRES_BIERES.map(n  => ({ id:n, name:n, builtin:true })),
      boissons:       AUTRES_BOISSONS.map(n => ({ id:n, name:n, builtin:true })),
      nourriture:     NOURRITURE.map(n      => ({ id:n, name:n, builtin:true })),
      autre:          [],
    };
    // Ajoute les produits custom dans leur catégorie
    customs.forEach(p => {
      const cat = p.category || "autre";
      if (!groups[cat]) groups[cat] = [];
      groups[cat].push({ id:p.id, name:p.name, builtin:false, price:p.price });
    });
    return groups;
  };
  ...
  const handleAdd = () => {
    ...
    const updated = isEdit
      ? customs.map(c => c.id===editId ? newProd : c)
      : [...customs, newProd];
    saveCustomProducts(updated);
    setCustomsState(updated);
    // Ajouter à l'inventaire si nouveau
    if (!isEdit) {
      setInventory(inv => ({ ...inv, regular: { ...inv.regular, [id]: 0 } }));
    }
    // Sauvegarder le prix
    const newPrices = { ...prices, [id]: form.price };
    setPricesState(newPrices);
    savePrices(newPrices);
    setForm({ name:"", category:"petites_bieres", price:"" });
    setEditId(null);
    showToast(isEdit ? "✅ Produit mis à jour!" : `✅ "${form.name.trim()}" ajouté partout!`, "ok");
    setSubTab("catalogue");
  };

  const handleDelete = (prod) => {
    if (prod.builtin) {
      showToast("⛔ Les produits de base ne peuvent pas être supprimés", "err");
      return;
    }
    if (!window.confirm(`Supprimer "${prod.name}" définitivement ?`)) return;
  ...
  const groups = allProductsByCategory();
    ...
    <>
      <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
        {[
          ...
          {v:"prix",      l:"💲 Modifier les prix"},
          {v:"ajouter",   l:"➕ Ajouter un produit"},
        ].map(t => (
          <button key={t.v} className={`btn ${subTab===t.v?"btn-accent":"btn-ghost"}`}
            onClick={()=>{setSubTab(t.v);setEditId(null);setForm({name:"",category:"petites_bieres",price:""});}}>
            {t.l}
          </button>
        ))}
      </div>
      ...
      {subTab === "catalogue" && (
        <>
          <div style={{fontSize:".8rem",color:"var(--muted)",marginBottom:12}}>
            Tous vos produits. Les produits personnalisés peuvent être modifiés ou supprimés.
          </div>
          {CATEGORIES.map(cat => {
            const items = groups[cat.value] || [];
            if (items.length === 0) return null;
            return (
              <div className="card" key={cat.value} style={{marginBottom:10}}>
                <div className="card-header">
                  <h3>{cat.label}</h3>
                ...
                <div className="card-body" style={{padding:0}}>
                  <table>
                    <thead>
                      <tr>
                        <th>Produit</th>
                        <th style={{textAlign:"center",width:80}}>Prix</th>
                    ...
                    <tbody>
                      {items.map(prod => {
                        const price = parseFloat(prices[prod.id]) || 0;
                        const stock = inventory.regular?.[prod.id] ?? 0;
                        return (
                          <tr key={prod.id}>
                            <td className="item-name" style={{fontSize:".85rem"}}>
                              {prod.name}
                              {!prod.builtin && (
                                <span style={{fontSize:".68rem",background:"rgba(232,160,32,.15)",
                                  color:"var(--accent)",borderRadius:4,padding:"1px 5px",marginLeft:6}}>
                                  custom
                                </span>
                              )}
                            </td>
                            <td style={{textAlign:"center",fontSize:".82rem",
                              color: price===0?"var(--red)":"var(--muted)"}}>
                              {price===0 ? "—" : `${price.toFixed(2)} $`}
                            </td>
                            <td style={{textAlign:"center",fontSize:".82rem",
                              color: stock<=5?"var(--orange)":"var(--muted)"}}>
                              ...
                              {!prod.builtin && (
                                <div style={{display:"flex",gap:4,justifyContent:"center"}}>
                                  <button className="btn btn-ghost"
                                    ...
                                    onClick={()=>{
                                      const p = customs.find(c=>c.id===prod.id);
                                      setEditId(prod.id);
                                      setForm({name:prod.name,category:p?.category||"autre",price:prices[prod.id]||""});
                                      setSubTab("ajouter");
                                    }}>✏️</button>
                                  <button className="btn btn-red"
                                    style={{fontSize:".7rem",padding:"3px 8px"}}
                                    onClick={()=>handleDelete(prod)}>🗑</button>
                                </div>
                              )}
                            </td>
                          </tr>
      ...
      {subTab === "prix" && (<>
        <div style={{display:"flex",justifyContent:"flex-end",marginBottom:10}}>
          <button className="btn btn-green" onClick={handleSavePrices}>💾 Sauvegarder tous les prix</button>
        </div>
        {[
          { items:PETITES_BIERES,  label:"🍺 Petites Bières",   gk:n=>n, gl:n=>n },
          { items:GROSSES_BIERES,  label:"🍺 Grosses Bières",   gk:n=>n, gl:n=>n },
          { items:AUTRES_BIERES,   label:"🍻 Autres Bières",    gk:n=>n, gl:n=>n },
          { items:BUG_ITEMS,       label:"🥃 Forts",            gk:b=>b.id, gl:b=>`${b.label} (${b.oz} oz)` },
          { items:VIN_ITEMS,       label:"🍷 Vins & Spiritueux", gk:v=>v.id, gl:v=>v.label },
          { items:AUTRES_BOISSONS, label:"🥤 Autres Boissons",  gk:n=>n, gl:n=>n },
          { items:NOURRITURE,      label:"🍗 Nourriture",       gk:n=>n, gl:n=>n },
        ].map(({ items, label, gk, gl }) => (
          <div className="card" key={label} style={{marginBottom:10}}>
              ...
              <table>
                ...
                <tbody>
                  {items.map(item => {
                    ...
                    return (
                      <tr key={key}>
                        <td className="item-name">{gl(item)}</td>
                        <td>
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                  ...
                  {/* Custom products in this category */}
                  {customs.filter(p => {
                    const catMap = {petites_bieres:"petites_bieres",grosses_bieres:"grosses_bieres",
                      autres_bieres:"autres_bieres",boissons:"boissons",nourriture:"nourriture"};
                    const catLabel = label.includes("Petite")?"petites_bieres":
                      label.includes("Grosse")?"grosses_bieres":
                      label.includes("Autre Bière")||label.includes("Autres Bières")&&!label.includes("Boisson")?"autres_bieres":
                      label.includes("Boisson")?"boissons":
                      label.includes("Nourriture")?"nourriture":"autre";
                    return p.category === catLabel;
                  }).map(p => (
                    <tr key={p.id} style={{background:"rgba(232,160,32,.04)"}}>
                      <td className="item-name">
                        {p.name}
                        <span style={{fontSize:".68rem",color:"var(--accent)",marginLeft:6}}>custom</span>
                      </td>
                      <td>
                        <div style={{display:"flex",alignItems:"center",gap:6}}>
                          <span style={{color:"var(--muted)",fontSize:".82rem"}}>$</span>
      ...
      {subTab === "ajouter" && (
        <div className="card">
          ...
          <div className="card-body" style={{display:"flex",flexDirection:"column",gap:14,maxWidth:440}}>
            <div>
              <div style={{fontSize:".74rem",color:"var(--muted)",marginBottom:4}}>Nom du produit *</div>
              <input type="text" value={form.name}
                placeholder="Ex: Heineken 0 alcool, Nachos, Spritz..."
            ...
            <div>
              ...
              <input type="number" min="0" step="0.25" value={form.price}
                placeholder="0.00" style={{width:140}}
                onChange={e=>setForm(f=>({...f,price:e.target.value}))} />
            </div>
            <div style={{fontSize:".76rem",color:"var(--muted)",background:"var(--surface)",
              borderRadius:7,padding:"10px 12px",lineHeight:1.6}}>
              ✅ Le produit sera automatiquement ajouté à :<br/>
              • La feuille de vente des barmaids (section {CATEGORIES.find(c=>c.value===form.category)?.label})<br/>
              • L'inventaire (stock initial : 0)<br/>
            ...
            <div className="btn-row" style={{marginTop:0}}>
              ...
              <button className="btn btn-ghost"
                onClick={()=>{setSubTab("catalogue");setEditId(null);setForm({name:"",category:"petites_bieres",price:""});}}>
                Annuler
              </button>
            </div>
          </div>
...
function ManagerToday({ sheet, setSheet, prevFinsPM, onApply, anomalies, showToast }) {
  ...
  return (
    <>
      ...
      {/* ── OFFERTES PAR MATHIEU — validation gérant ── */}
      {(sheet.offertes||[]).length > 0 && (() => {
        const approve = (id, val) => {
          setSheet(s=>({...s, offertes:(s.offertes||[]).map(o=>o.id===id?{...o,approuve:val}:o)}));
          showToast(val ? "✅ Approuvée" : "❌ Refusée", val?"ok":"err");
        };
        const totalDeduit = (sheet.offertes||[]).filter(o=>o.approuve!==false).reduce((s,o)=>s+(parseFloat(o.prix)||0),0);
        return (
          <div className="card">
            <div className="card-header">
              <h3>🎁 Consommations offertes</h3>
              <span style={{marginLeft:"auto",fontSize:".78rem",color:"var(--orange)"}}>
                Total déduit : {totalDeduit.toFixed(2)} $
              </span>
            </div>
            <div className="card-body" style={{padding:0}}>
              <table>
                <thead><tr><th>Shift</th><th>Article</th><th>Qté</th><th>Prix</th><th>Note</th><th style={{textAlign:"center"}}>Décision</th></tr></thead>
                <tbody>
                  {(sheet.offertes||[]).map(o=>(
                    <tr key={o.id} style={{background:o.approuve===true?"rgba(34,197,94,.05)":o.approuve===false?"rgba(239,68,68,.05)":undefined}}>
                      <td><span className={`badge badge-${(o.shift||"AM").toLowerCase()}`}>{o.shift}</span></td>
                      <td className="item-name" style={{fontSize:".82rem"}}>{o.label}</td>
                      <td style={{textAlign:"center"}}>{o.qty}</td>
                      <td style={{textAlign:"right",color:"var(--orange)"}}>-{(parseFloat(o.prix)||0).toFixed(2)} $</td>
                      <td style={{fontSize:".75rem",color:"var(--muted)"}}>{o.note||"—"}</td>
                      <td style={{textAlign:"center"}}>
                        {o.approuve===null ? (
                          <div style={{display:"flex",gap:6,justifyContent:"center"}}>
                            <button className="btn btn-green" style={{padding:"3px 10px",fontSize:".75rem"}} onClick={()=>approve(o.id,true)}>✅</button>
                            <button className="btn btn-ghost" style={{padding:"3px 10px",fontSize:".75rem",color:"var(--red)"}} onClick={()=>approve(o.id,false)}>❌</button>
                          </div>
                        ) : (
                          <span style={{fontSize:".78rem",fontWeight:600,color:o.approuve?"var(--green)":"var(--red)"}}>
                            {o.approuve?"✅":"❌"}
                            <button style={{marginLeft:6,background:"none",border:"none",color:"var(--muted)",cursor:"pointer",fontSize:".7rem"}} onClick={()=>approve(o.id,null)}>⟳</button>
                          </span>
                        )}
                      </td>
                    </tr>
      ...
      {/* ── ALERTES ERREUR — validation gérant ── */}
      {(sheet.alertesErreur||[]).length > 0 && (() => {
        const decide = (id, val) => {
          setSheet(s=>({...s, alertesErreur:(s.alertesErreur||[]).map(a=>a.id===id?{...a,statut:val}:a)}));
          showToast(val ? "✅ Erreur validée" : "❌ Alerte refusée", val?"ok":"err");
        };
        return (
          <div className="card" style={{borderColor:"var(--orange)"}}>
            <div className="card-header"><h3>⚠️ Alertes d'erreur</h3></div>
            <div className="card-body" style={{display:"flex",flexDirection:"column",gap:8}}>
              {(sheet.alertesErreur||[]).map(a=>(
                <div key={a.id} style={{padding:"10px 12px",borderRadius:7,
                  background:a.statut===true?"rgba(34,197,94,.08)":a.statut===false?"rgba(239,68,68,.08)":"rgba(249,115,22,.08)",
                  border:`1px solid ${a.statut===true?"var(--green)":a.statut===false?"var(--red)":"var(--orange)"}`}}>
                  <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:4}}>
                    <span className={`badge badge-${(a.shift||"AM").toLowerCase()}`}>{a.shift}</span>
                    <span style={{fontSize:".78rem",fontWeight:600}}>{a.soumisePar}</span>
                    <span style={{fontSize:".72rem",color:"var(--muted)"}}>{a.ts}</span>
                  </div>
                  <div style={{fontSize:".88rem",marginBottom:8}}>{a.texte}</div>
                  {a.statut===null ? (
                    <div style={{display:"flex",gap:8}}>
                      <button className="btn btn-green" style={{padding:"3px 12px",fontSize:".78rem"}} onClick={()=>decide(a.id,true)}>✅ Valider</button>
                      <button className="btn btn-ghost" style={{padding:"3px 12px",fontSize:".78rem",color:"var(--red)"}} onClick={()=>decide(a.id,false)}>❌ Refuser</button>
                    </div>
                  ) : (
                    <span style={{fontSize:".78rem",fontWeight:600,color:a.statut?"var(--green)":"var(--red)"}}>
                      {a.statut?"✅ Validée":"❌ Refusée"}
                      <button style={{marginLeft:8,background:"none",border:"none",color:"var(--muted)",cursor:"pointer"}} onClick={()=>decide(a.id,null)}>⟳</button>
                    </span>
                  )}
                </div>
              ))}
...
function ManagerEmployes({ history, showToast }) {
  ...
  const [form, setForm] = useState({ name:"", pin:"", pin2:"", role:"barmaid", permissions:{ promos:false } });
  ...
  const openAdd  = () => { setForm({name:"",pin:"",pin2:"",role:"barmaid",permissions:{promos:false}}); setPinErr(""); setView("add"); };
  const openEdit = (emp) => {
    setEditTarget(emp);
    setForm({
      name: emp.name,
      pin:  emp.pin,
      pin2: emp.pin,
      role: emp.role || "barmaid",
      permissions: {
        promos: emp.permissions?.promos === true, // false si undefined
      }
    });
  ...
  const handleAdd = () => {
    if (!validate()) return;
    const newEmp = { id:Date.now().toString(), name:form.name.trim(), pin:form.pin, role:form.role, permissions:form.permissions };
  ...
  const handleEdit = () => {
    ...
    persist(employees.map(e => e.id===editTarget.id
      ? { ...e, name:form.name.trim(), pin:form.pin, role:form.role, permissions:form.permissions }
      : e));
      ...
      <>
        ...
        <div className="card">
          <div className="card-body" style={{display:"flex",flexDirection:"column",gap:16,maxWidth:420}}>
            ...
            {form.role === "barmaid" ? (
              <div>
                <div style={{fontSize:".75rem",color:"var(--muted)",marginBottom:8,fontWeight:600,
                  textTransform:"uppercase",letterSpacing:".05em"}}>
                  Permissions spéciales
                </div>
                <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",
                  background:"var(--surface)",borderRadius:7,padding:"10px 12px",
                  border:`1px solid ${form.permissions?.promos?"var(--accent)":"var(--border)"}`}}>
                  <input type="checkbox"
                    checked={!!form.permissions?.promos}
                    onChange={e=>setForm(f=>({...f,permissions:{...f.permissions,promos:e.target.checked}}))}
                    style={{width:16,height:16,accentColor:"var(--accent)"}} />
                  <div>
                    <div style={{fontSize:".85rem",fontWeight:600}}>🏷 Peut appliquer des promos</div>
                    <div style={{fontSize:".7rem",color:"var(--muted)"}}>
                      Promo Shooter 2$ · Promo Petite Bière 4$ · Promo Vin 5$
                    </div>
                  </div>
                </label>
              </div>
              ...
              <div style={{background:"var(--surface)",borderRadius:7,padding:"10px 12px",
                border:"1px solid var(--border)",fontSize:".82rem",color:"var(--muted)"}}>
                🏷 Promos — accès automatique ({form.role === "gerant" ? "Gérant" : "Manageuse"})
              </div>
            )}
                ...
                <div style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:".73rem",color:"var(--muted)",marginTop:4,display:"flex",gap:10,flexWrap:"wrap"}}>
                      ...
                      {canUsePromos(emp) && (
                        <span style={{color:"var(--accent)",fontStyle:"normal"}}>🏷 Promos</span>
                      )}
                    </div>
                  </div>
...
function ManagerLoto({ today, history }) {
  const [viewMode, setViewMode] = useState("semaine");
  const [selectedWeek, setSelectedWeek] = useState(() => {
    const d = new Date(today+"T12:00:00");
    const day = d.getDay();
  ...
  const fmtFull = (ds) => {
    ...
    return `${j[d.getDay()]} ${d.getDate()} ${monthNames[d.getMonth()]}`;
  };

  const getDates = () => {
    if (viewMode==="semaine") return Array.from({length:7},(_,i)=>addDays(selectedWeek,i));
    if (viewMode==="mois") {
      const [y,m]=selectedMonth.split("-").map(Number);
      return Array.from({length:new Date(y,m,0).getDate()},(_,i)=>`${selectedMonth}-${String(i+1).padStart(2,"0")}`);
    }
  ...
  history.forEach(d=>{if(d?.date)histMap[d.date]=d;});
  dates.forEach(date=>{
    if(!histMap[date]){const s=load("sheet_"+date,null);if(s)histMap[date]=migrateSheet(s);}
  });

  // Colonnes : Machines (dépôt lotoQuebec) | Coupons AM | Coupons PM | Total Coupons | Net
  const rows = dates.map(date => {
    const depot   = loadDepot(date);
    const sheet   = histMap[date];
    const machines   = parseFloat(depot.lotoQuebec)||0;
    const couponsAM  = parseFloat(sheet?.caisse?.AM?.coupons)||0;
    const couponsPM  = parseFloat(sheet?.caisse?.PM?.coupons)||0;
    const totCoupons = couponsAM + couponsPM;
    const net        = machines - totCoupons;
    const hasData    = machines > 0 || totCoupons > 0;
    return { date, fmtDate:fmtFull(date), machines, couponsAM, couponsPM, totCoupons, net, hasData, isToday:date===today };
  }).filter(r => r.hasData || r.isToday);

  const totMachines = rows.reduce((s,r)=>s+r.machines,0);
  const totCoupons  = rows.reduce((s,r)=>s+r.totCoupons,0);
  const totNet      = totMachines - totCoupons;
  const fmt = (n) => n>0?`${n.toFixed(2)} $`:"—";

  return (
    <>
      <div className="card">
        <div className="card-header"><h3>🎰 Suivi Loto-Québec</h3></div>
        <div className="card-body">
          <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
            {[{v:"semaine",l:"Semaine"},{v:"mois",l:"Mois"},{v:"30jours",l:"30 jours"}].map(p=>(
              <button key={p.v} className={`btn ${viewMode===p.v?"btn-accent":"btn-ghost"}`}
                onClick={()=>setViewMode(p.v)}>{p.l}</button>
            ))}
          </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:10,marginBottom:14}}>
        {[
          {label:"🎰 Total Machines", val:totMachines, color:"var(--accent)"},
          {label:"🎟 Total Coupons",  val:totCoupons,  color:"var(--orange)"},
          {label:"💚 Net",            val:totNet,       color:"var(--green)"},
        ].map(({label,val,color})=>(
          <div key={label} style={{background:"var(--card)",border:"1px solid var(--border)",
            borderRadius:8,padding:"12px 14px",textAlign:"center"}}>
            <div style={{fontSize:".72rem",color:"var(--muted)",marginBottom:4}}>{label}</div>
            ...
            <table>
              <thead>
                <tr>
                  <th>Date</th>
                  <th style={{textAlign:"right"}}>🎰 Machines</th>
                  <th style={{textAlign:"right"}}>🎟 Coupons AM</th>
                  <th style={{textAlign:"right"}}>🎟 Coupons PM</th>
                  <th style={{textAlign:"right"}}>Total coupons</th>
                  <th style={{textAlign:"right"}}>= Net</th>
                </tr>
              </thead>
              <tbody>
                {rows.map(r=>(
                  <tr key={r.date} style={{background:r.isToday?"rgba(232,160,32,.04)":undefined}}>
                    <td style={{fontSize:".82rem",fontWeight:r.isToday?700:400}}>
                      {r.fmtDate}
                      {r.isToday&&<span style={{fontSize:".65rem",color:"var(--accent)",marginLeft:6}}>aujourd'hui</span>}
                    </td>
                    ...
                    <td style={{textAlign:"right",color:r.couponsAM>0?"var(--orange)":"var(--muted)"}}>
                      {r.couponsAM>0?`${r.couponsAM.toFixed(2)} $`:"—"}
                    </td>
                    <td style={{textAlign:"right",color:r.couponsPM>0?"var(--orange)":"var(--muted)"}}>
                      {r.couponsPM>0?`${r.couponsPM.toFixed(2)} $`:"—"}
                    </td>
                    <td style={{textAlign:"right",fontWeight:600,color:r.totCoupons>0?"var(--orange)":"var(--muted)"}}>
                      {r.totCoupons>0?`${r.totCoupons.toFixed(2)} $`:"—"}
                    </td>
                    <td style={{textAlign:"right",fontWeight:700,
                      color:r.net>=0?"var(--green)":"var(--red)"}}>
                      {r.hasData?`${r.net.toFixed(2)} $`:"—"}
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{borderTop:"2px solid var(--border)",background:"var(--surface)"}}>
                  <td style={{fontWeight:700,padding:"10px 16px",fontSize:".82rem"}}>
                    TOTAL ({rows.length} jour{rows.length>1?"s":""})
                  </td>
                  <td style={{textAlign:"right",fontWeight:700,color:"var(--accent)"}}>
                    {totMachines.toFixed(2)} $
                  </td>
                  <td style={{textAlign:"right",fontWeight:700,color:"var(--orange)"}}>
                    {rows.reduce((s,r)=>s+r.couponsAM,0).toFixed(2)} $
                  </td>
                  <td style={{textAlign:"right",fontWeight:700,color:"var(--orange)"}}>
                    {rows.reduce((s,r)=>s+r.couponsPM,0).toFixed(2)} $
                  </td>
                  <td style={{textAlign:"right",fontWeight:700,color:"var(--orange)"}}>
                    {totCoupons.toFixed(2)} $
                  </td>
                  <td style={{textAlign:"right",fontWeight:700,color:totNet>=0?"var(--green)":"var(--red)"}}>
                    {totNet.toFixed(2)} $
                  </td>
                </tr>
              </tfoot>
...
const APP_PASSWORD = "clubdesex2024";
function AppPasswordGate({ onUnlock }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"#0f0f0f",gap:24,padding:20}}>
      <div style={{fontFamily:"Georgia,serif",fontSize:"2rem",color:"#e8a020",letterSpacing:".1em"}}>LE CLUB DES EX</div>
      <div style={{color:"#888",fontSize:".9rem"}}>Entrez le mot de passe pour accéder</div>
      <div style={{display:"flex",flexDirection:"column",gap:12,width:"100%",maxWidth:300}}>
        <input type="password" value={input} placeholder="Mot de passe..." autoFocus
          style={{fontSize:"1.1rem",padding:"12px 16px",textAlign:"center",background:"#1a1a1a",border:"1px solid "+(error?"#ef4444":"#333"),borderRadius:8,color:"#fff",outline:"none"}}
          onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&tryUnlock()} />
        {error && <div style={{color:"#ef4444",textAlign:"center",fontSize:".82rem"}}>⛔ Mot de passe incorrect</div>}
        <button style={{padding:"12px",fontSize:"1rem",background:"#e8a020",border:"none",borderRadius:8,color:"#000",fontWeight:700,cursor:"pointer"}} onClick={tryUnlock}>Entrer →</button>
      </div>
    </div>
  );
}
export default function App() {
  ...
  const [appUnlocked, setAppUnlocked] = useState(() => sessionStorage.getItem("app_unlocked") === "1");
  ...
  useEffect(() => {
    ...
    const init=async()=>{
      try{
        await initAuth();
        const localKeys=Object.keys(localStorage).filter(k=>k.startsWith("sheet_")||k.startsWith("depot_"));
        const syncKeys=[...new Set(["manager_pin","employees","inventory","history","prices","custom_products","cocktails_v2","sheet_"+today,...localKeys])];
        for(const key of syncKeys){try{const val=await fbGet(key);if(val!==null){const cur=localStorage.getItem(key);if(JSON.stringify(val)!==cur)localStorage.setItem(key,JSON.stringify(val));}}catch{}}
        ["sheet_"+today,"employees","inventory","prices","manager_pin"].forEach(key=>{
          const unsub=fbListen(key,(val)=>{const cur=localStorage.getItem(key);if(JSON.stringify(val)!==cur){localStorage.setItem(key,JSON.stringify(val));if(key==="sheet_"+today||key==="employees")window.location.reload();}});
          unsubRef.current.push(unsub);
        });
      }catch(e){console.warn("[Firebase] Hors-ligne:",e.message);}
    };
    init();
    return()=>{window.removeEventListener("online",onOnline);window.removeEventListener("offline",onOffline);unsubRef.current.forEach(u=>u());};
  },[]);
  ...
  // Re-sync debutAM from prevFinsPM whenever Firebase loads new data
  // This ensures the tracking works even after a sync from another device
  useEffect(() => {
    if (!prevFinsPM) return;
    setSheetRaw(s => {
      // Only update if today's sheet exists and debutAM values are missing
      let changed = false;
      const newS = JSON.parse(JSON.stringify(s));
      BUG_ITEMS.forEach(b => {
        const fin = prevFinsPM.bugs?.[b.id];
        if (fin !== undefined && fin !== "" && (newS.bugs[b.id]?.debutAM === "" || newS.bugs[b.id]?.debutAM === undefined)) {
          newS.bugs[b.id].debutAM = fin;
          changed = true;
        }
      });
      VIN_ITEMS.forEach(v => {
        const fin = prevFinsPM.vins?.[v.id];
        if (fin !== undefined && fin !== "" && (newS.vins[v.id]?.debutAM === "" || newS.vins[v.id]?.debutAM === undefined)) {
          newS.vins[v.id].debutAM = fin;
          changed = true;
        }
      });
      if (changed) save("sheet_"+today, newS);
  ...
  const [history, setHistoryRaw] = useState(() => {
    // Load from "history" array + all individual "sheet_YYYY-MM-DD" keys
    const fromHistory = (load("history", []) || []).map(d => migrateSheet(d));
    const histMap = {};
    fromHistory.forEach(d => { if (d?.date) histMap[d.date] = d; });
    // Also scan localStorage for any sheet_ keys not already in history
    Object.keys(localStorage).forEach(k => {
      if (k.startsWith("sheet_") && k !== "sheet_"+TODAY()) {
        const date = k.replace("sheet_", "");
        if (!histMap[date]) {
          try {
            const s = migrateSheet(JSON.parse(localStorage.getItem(k)));
            if (s?.date) histMap[s.date] = s;
          } catch {}
        }
      }
    });
    return Object.values(histMap).sort((a,b) => b.date.localeCompare(a.date));
  });
  ...
  return (
    <>
      <style>{CSS}</style>
      {!online&&<div style={{position:"fixed",top:0,left:0,right:0,zIndex:9999,background:"#b45309",color:"#fff",textAlign:"center",fontSize:".75rem",padding:"5px",fontWeight:600}}>📵 Hors-ligne — synchronisation automatique à la reconnexion</div>}
      {!appUnlocked?(
        <AppPasswordGate onUnlock={()=>{sessionStorage.setItem("app_unlocked","1");setAppUnlocked(true);}}/>
      ):(<>
      {screen==="roleSelect" && (
        <RoleSelect
        ...
        <div style={{background:"var(--bg)",minHeight:"100vh"}}>
          <PinLogin
            ...
            onSuccess={np => {
              ...
              if (ok) { unlockManager(); setScreen("depot"); }
              return ok;
            }}
        ...
        <div style={{background:"var(--bg)",minHeight:"100vh"}}>
          <PinLogin
            ...
            onSuccess={np => {
              const ok = checkManagerPin(np);
              if (ok) {
                // Identify which employee logged in
                const employees = loadEmployees();
                const matched = employees.find(e => e.pin === np);
                if (matched) setCurrentEmployee(matched);
                setScreen("manager");
              }
              return ok;
            }}
