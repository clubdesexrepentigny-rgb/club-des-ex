    ...
    <div className="app">
      ...
      <div className="main">
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div className="bug-grid">
              {VIN_ITEMS.map(vin => renderCounterCard(vin, "vins", sheet.vins[vin.id]||{}, setVin, onFinAMBlurVin))}
            </div>
          </div>
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        await initAuth();
        const keys = ["manager_pin","employees","inventory","history","prices","custom_products","cocktails_v2","sheet_"+today];
        for (const key of keys) {
          try { const val = await fbGet(key); if (val !== null) localStorage.setItem(key, JSON.stringify(val)); } catch {}
        }
        ["sheet_"+today, "employees", "inventory"].forEach(key => {
          const unsub = fbListen(key, (val) => {
            const cur = localStorage.getItem(key);
            if (JSON.stringify(val) !== cur) { localStorage.setItem(key, JSON.stringify(val)); window.location.reload(); }
          });
  ...
  return (
    <>
      <style>{CSS}</style>
      {!online && <div style={{position:"fixed",top:0,left:0,right:0,zIndex:9999,background:"#b45309",color:"#fff",textAlign:"center",fontSize:".75rem",padding:"4px",fontWeight:600}}>📵 Hors-ligne</div>}
      {!appUnlocked ? (
